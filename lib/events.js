...
events.types = ['plugin-activate', 'plugin-deactivate', 'plugin-install', 'plugin-uninstall', 'restart', 'build', 'config-change', 'settings-change', 'category-purge', 'privilege-change', 'post-delete', 'post-restore', 'post-purge', 'post-edit', 'post-move', 'post-change-owner', 'post-queue-reply-accept', 'post-queue-topic-accept', 'post-queue-reply-reject', 'post-queue-topic-reject', 'topic-delete', 'topic-restore', 'topic-purge', 'topic-rename', 'topic-merge', 'topic-fork', 'topic-move', 'topic-move-all', 'password-reset', 'user-makeAdmin', 'user-removeAdmin', 'user-ban', 'user-unban', 'user-mute', 'user-unmute', 'user-delete', 'user-deleteAccount', 'user-deleteContent', 'password-change', 'email-confirmation-sent', 'email-change', 'username-change', 'ip-blacklist-save', 'ip-blacklist-addRule', 'registration-approved', 'registration-rejected', 'group-join', 'group-request-membership', 'group-add-member', 'group-leave', 'group-owner-grant', 'group-owner-rescind', 'group-accept-membership', 'group-reject-membership', 'group-invite', 'group-invite-accept', 'group-invite-reject', 'group-kick', 'theme-set', 'export:uploads', 'account-locked', 'getUsersCSV', 'getGroupCSV', 'chat-room-deleted'];
events.log = async function (data) {
  ...
  await Promise.all([db.sortedSetsAdd(setKeys, data.timestamp, eid), db.setObject(`event:${eid}`, data)]);
  plugins.hooks.fire('action:events.log', {
    data: data
  });
};
events.getEvents = async function (options) {
  ...
  if (Array.isArray(uids)) {
    if (filter === '') {
      eids = await db.getSortedSetRevRangeByScore(uids.map(uid => `events:time:uid:${uid}`), start, stop === -1 ? -1 : stop - start + 1, to, from);
    } else {
      eids = await Promise.all(uids.map(uid => db.getSortedSetRevIntersect({
      ...
      eids = _.flatten(eids).filter(i => (from === '-inf' || i.score >= from) && (to === '+inf' || i.score <= to)).sort((a, b) => b.score - a.score).slice(start, stop + 1).map(i => i.value);
    }
  } else {
    eids = await db.getSortedSetRevRangeByScore(`events:time${filter ? `:${filter}` : ''}`, start, stop === -1 ? -1 : stop - start + 1, to, from);
  }
...
events.getEventCount = async options => {
  ...
  if (Array.isArray(uids)) {
    if (filter === '') {
      const counts = await Promise.all(uids.map(uid => db.sortedSetCount(`events:time:uid:${uid}`, from, to)));
      return counts.reduce((prev, cur) => prev + cur, 0);
    }
    const eids = await Promise.all(uids.map(uid => db.getSortedSetRevIntersect({
      sets: [`events:time:uid:${uid}`, `events:time:${filter}`],
      start: 0,
      stop: -1,
      weights: [1, 0],
    ...
    return _.flatten(eids).filter(i => (from === '-inf' || i.score >= from) && (to === '+inf' || i.score <= to)).length;
  }
...
events.deleteEvents = async function (eids) {
  ...
  const sets = _.uniq(['events:time'].concat(eventData.map(e => `events:time:${e.type}`)).concat(eventData.map(e => `events:time:uid:${e.uid}`)));
