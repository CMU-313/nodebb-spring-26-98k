...
module.exports = function (User) {
  ...
  User.bans.unban = async function (uids, reason = '') {
    ...
    await db.setObject(uids.map(uid => `user:${uid}`), {
      banned: 0,
      'banned:expire': 0
    });
    const now = Date.now();
    ...
    for (const user of userData) {
      ...
      await Promise.all([db.sortedSetAdd(`uid:${user.uid}:unbans:timestamp`, now, unbanKey), db.setObject(unbanKey, unbanData), groups.leave(groups.BANNED_USERS, user.uid), groups.join(systemGroupsToJoin, user.uid)]);
  ...
  User.bans.canLoginIfBanned = async function (uid) {
    ...
    const {
      banned
    } = (await User.bans.unbanIfExpired([uid]))[0];
    if (banned) {
