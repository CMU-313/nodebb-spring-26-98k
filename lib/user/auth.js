...
module.exports = function (User) {
  ...
  async function cleanExpiredSessions(uid) {
    ...
    await Promise.all(sids.map(async sid => {
      ...
      const expired = !sessionObj || !sessionObj.hasOwnProperty('passport') || !sessionObj.passport.hasOwnProperty('user') || parseInt(sessionObj.passport.user, 10) !== parseInt(uid, 10);
      if (expired) {
  ...
  User.auth.revokeSession = async function (sessionIds, uid) {
    ...
    await Promise.all([db.sortedSetRemove(`uid:${uid}:sessions`, sessionIds), destroySids(sessionIds)]);
  };
  ...
  User.auth.deleteAllSessions = async function () {
    await batch.processSortedSet('users:joindate', async uids => {
      ...
      await Promise.all([db.deleteAll(sessionKeys), ...sids.map(sid => db.sessionStoreDestroy(sid))]);
    }, {
      batch: 1000
    });
  };
