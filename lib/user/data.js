...
module.exports = function (User) {
  const fieldWhitelist = ['uid', 'username', 'userslug', 'url', 'email', 'email:confirmed', 'joindate', 'lastonline', 'picture', 'icon:bgColor', 'fullname', 'birthday', 'aboutme', 'signature', 'uploadedpicture', 'profileviews', 'reputation', 'postcount', 'topiccount', 'lastposttime', 'banned', 'banned:expire', 'status', 'flags', 'followerCount', 'followingCount', 'cover:url', 'cover:position', 'groupTitle', 'mutedUntil', 'mutedReason'];
  ...
  User.getUserFieldWhitelist = async function () {
    const {
      whitelist
    } = await plugins.hooks.fire('filter:user.whitelistFields', {
      uids: [],
  ...
  User.getUsersFields = async function (uids, fields) {
    ...
    const users = await db.getObjectsFields(uniqueUids.map(uid => `user:${uid}`).concat(remoteIds.map(id => `userRemote:${id}`)), fields);
  ...
  async function modifyUserData(users, requestedFields, fieldsToRemove) {
    ...
    if (meta.config.showFullnameAsDisplayName) {
      ...
      uidToSettings = _.zipObject(uids, await db.getObjectsFields(uids.map(uid => `user:${uid}:settings`), ['showfullname']));
    }
    if (!iconBackgrounds) {
  ...
  User.getIconBackgrounds = async () => {
    ...
    const _iconBackgrounds = ['#f44336', '#e91e63', '#9c27b0', '#673ab7', '#3f51b5', '#2196f3', '#009688', '#1b5e20', '#33691e', '#827717', '#e65100', '#ff5722', '#795548', '#607d8b'];
