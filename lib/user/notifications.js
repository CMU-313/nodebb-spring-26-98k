...
UserNotifications.getAllWithCounts = async function (uid, filter) {
  ...
  return {
    counts,
    nids: notifications.map(n => n.nid)
  };
};
async function getAllNids(uid) {
  let nids = await db.getSortedSetRevRange([`uid:${uid}:notifications:unread`, `uid:${uid}:notifications:read`], 0, -1);
  nids = _.uniq(nids);
...
UserNotifications.getNotifications = async function (nids, uid) {
  ...
  const [notifObjs, hasRead, userSettings] = await Promise.all([notifications.getMultiple(nids), db.isSortedSetMembers(`uid:${uid}:notifications:read`, nids), user.getSettings(uid)]);
  const deletedNids = [];
...
UserNotifications.getUnreadCount = async function (uid) {
  ...
  ({
    count
  } = await plugins.hooks.fire('filter:user.notifications.getCount', {
    uid,
    count
  }));
  return count;
...
UserNotifications.sendTopicNotificationToFollowers = async function (uid, topicData, postData) {
  try {
    const [allFollowers, title] = await Promise.all([db.getSortedSetRange(`followers:${uid}`, 0, -1), topics.getTopicField(topicData.tid, 'title')]);
