...
Interstitials.email = async data => {
  ...
  const [hasPassword, hasPending] = await Promise.all([user.hasPassword(data.userData.uid), user.email.isValidationPending(data.userData.uid)]);
  let email;
    ...
    callback: async (userData, formData) => {
      ...
      if (userData.uid) {
        ...
        const [isPasswordCorrect, canEdit, {
          ...
          allowed,
          error
        }] = await Promise.all([user.isPasswordCorrect(userData.uid, formData.password, data.req.ip), privileges.users.canEdit(data.req.uid, userData.uid), user.getUserFields(userData.uid, ['email', 'email:confirmed']), plugins.hooks.fire('filter:user.saveEmail', {
          uid: userData.uid,
        ...
        const {
          allowed,
          error
        } = await plugins.hooks.fire('filter:user.saveEmail', {
          uid: null,
          email: formData.email,
          registration: true,
          allowed: true,
