...
module.exports = function (User) {
  ...
  User.deleteAccount = async function (uid) {
    ...
    const bulkRemove = [['username:uid', userData.username], ['username:sorted', `${userData.username.toLowerCase()}:${uid}`], ['userslug:uid', userData.userslug], ['fullname:uid', userData.fullname]];
    if (userData.email) {
    ...
    await Promise.all([db.sortedSetRemoveBulk(bulkRemove), utils.isNumber(uid) ? db.decrObjectField('global', 'userCount') : null, db.deleteAll(keys), db.setRemove('invitation:uids', uid), deleteUserIps(uid), deleteUserFromFollowers(uid), deleteUserFromFollowedTopics(uid), deleteUserFromIgnoredTopics(uid), deleteUserFromFollowedTags(uid), deleteImages(uid), groups.leaveAllGroups(uid), flags.resolveFlag('user', uid, uid), User.reset.cleanByUid(uid), User.email.expireValidation(uid), activitypub.actors.remove(uid)]);
    await db.deleteAll([`followers:${uid}`, `following:${uid}`, `uid:${uid}:followed_tags`, `uid:${uid}:followed_tids`, `uid:${uid}:ignored_tids`, `${utils.isNumber(uid) ? 'user' : 'userRemote'}:${uid}`]);
    delete deletesInProgress[uid];
    return userData;
  };
  ...
  async function deleteChats(uid) {
    const roomIds = await db.getSortedSetRange([`uid:${uid}:chat:rooms`, `chat:rooms:public`], 0, -1);
  ...
  async function deleteUserFromFollowers(uid) {
    ...
    async function updateCount(uids, name, fieldName) {
      await batch.processArray(uids, async uids => {
        ...
        const bulkSet = counts.map((count, index) => [`user:${uids[index]}`, {
          [fieldName]: count || 0
        }]);
    ...
    await Promise.all([updateCount(following, 'followers:', 'followerCount'), updateCount(followers, 'following:', 'followingCount')]);
  }
