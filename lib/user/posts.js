...
module.exports = function (User) {
  ...
  async function isReady(uid, cid, field) {
    ...
    const [userData, isAdminOrMod, isMemberOfExempt] = await Promise.all([User.getUserFields(uid, ['uid', 'mutedUntil', 'joindate', 'email', 'reputation'].concat([field])), privileges.categories.isAdminOrMod(cid, uid), groups.isMemberOfAny(uid, meta.config.groupsExemptFromNewUserRestrictions)]);
    if (!userData.uid) {
    ...
    const {
      shouldIgnoreDelays
    } = await plugins.hooks.fire('filter:user.posts.isReady', {
    ...
    if (!isMemberOfExempt && meta.config.newbiePostDelay > 0 && meta.config.newbieReputationThreshold > userData.reputation && now - lasttime < meta.config.newbiePostDelay * 1000) {
  ...
  User.updatePostCount = async uids => {
    ...
    if (uids.length) {
      ...
      await Promise.all([db.setObjectBulk(uids.map((uid, index) => [`user${activitypub.helpers.isUri(uid) ? 'Remote' : ''}:${uid}`, {
