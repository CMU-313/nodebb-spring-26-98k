...
module.exports = function (User) {
  User.updateProfile = async function (uid, data, extraFields) {
    let fields = ['username', 'email', 'fullname', 'groupTitle', 'birthday', 'signature', 'aboutme', ...(await db.getSortedSetRange('user-custom-fields', 0, -1))];
  ...
  async function isUsernameAvailable(data, uid) {
    ...
    const {
      error
    } = await plugins.hooks.fire('filter:username.check', {
  ...
  User.checkUsername = async username => isUsernameAvailable({
    username
  });
  ...
  async function updateUsername(uid, newUsername, callerUid) {
    ...
    const now = Date.now();
    await Promise.all([updateUidMapping('username', uid, newUsername, userData.username), updateUidMapping('userslug', uid, newUserslug, userData.userslug), db.sortedSetAdd(`user:${uid}:usernames`, now, `${newUsername}:${now}:${callerUid}`)]);
  ...
  User.changePassword = async function (uid, data) {
    ...
    await Promise.all([User.setUserFields(data.uid, {
