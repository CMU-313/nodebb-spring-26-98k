...
module.exports = function (User) {
  User.setCategoryWatchState = async function (uid, cids, state) {
    ...
    const follows = cids.filter(cid => !utils.isNumber(cid)).map(cid => apiMethod('uid', uid, cid));
    await Promise.all([db.sortedSetsAdd(cids.map(cid => `cid:${cid}:uid:watch:state`), state, uid), ...follows]);
  };
  ...
  User.getCategoriesByStates = async function (uid, states) {
    const [localCids, remoteCids] = await Promise.all([categories.getAllCidsFromSet('categories:cid'), meta.config.activitypubEnabled ? db.getObjectValues('handle:cid') : []]);
