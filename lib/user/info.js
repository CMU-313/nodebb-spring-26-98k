...
module.exports = function (User) {
  ...
  User.getModerationHistory = async function (uid) {
    let [flags, bans, mutes] = await Promise.all([db.getSortedSetRevRangeWithScores(`flags:byTargetUid:${uid}`, 0, 19), db.getSortedSetRevRange([`uid:${uid}:bans:timestamp`, `uid:${uid}:unbans:timestamp`], 0, 19), db.getSortedSetRevRange([`uid:${uid}:mutes:timestamp`, `uid:${uid}:unmutes:timestamp`], 0, 19)]);
    ...
    [flags, bans, mutes] = await Promise.all([getFlagMetadata(payload), formatBanMuteData(bans, '[[user:info.banned-no-reason]]'), formatBanMuteData(mutes, '[[user:info.muted-no-reason]]')]);
    return {
      flags: flags,
      bans: bans,
      mutes: mutes
