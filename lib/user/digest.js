...
Digest.send = async function (data) {
  ...
  await batch.processArray(data.subscribers, async uids => {
    ...
    await Promise.all(userData.map(async (userObj, index) => {
      ...
      const [publicRooms, notifications, topics] = await Promise.all([getUnreadPublicRooms(userObj.uid), user.notifications.getUnreadInterval(userObj.uid, data.interval), getTermTopics(data.interval, userObj.uid)]);
      const unreadNotifs = notifications.filter(Boolean);
      if (!unreadNotifs.length && !topics.top.length && !topics.popular.length && !topics.recent.length && !publicRooms.length) {
        return;
      }
...
Digest.getDeliveryTimes = async (start, stop) => {
  ...
  const [scores, settings] = await Promise.all([db.sortedSetScores('digest:delivery', uids), Digest.getUsersInterval(uids)]);
...
async function getTermTopics(term, uid) {
  ...
  const popular = data.topics.filter(t => t.postcount > 1).sort((a, b) => b.postcount - a.postcount).slice(0, 10);
  const popularTids = popular.map(t => t.tid);
  const top = data.topics.filter(t => t.votes > 0 && !popularTids.includes(t.tid)).sort((a, b) => b.votes - a.votes).slice(0, 10);
  const topTids = top.map(t => t.tid);
  const recent = data.topics.filter(t => !topTids.includes(t.tid) && !popularTids.includes(t.tid)).sort((a, b) => b.lastposttime - a.lastposttime).slice(0, 10);
