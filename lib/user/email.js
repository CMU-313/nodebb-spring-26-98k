...
UserEmail.remove = async function (uid, sessionId) {
  ...
  await Promise.all([user.setUserFields(uid, {
    email: '',
    'email:confirmed': 0
  }), db.sortedSetRemoveBulk([['email:uid', email.toLowerCase()], ['email:sorted', `${email.toLowerCase()}:${uid}`]]), user.email.expireValidation(uid), sessionId ? user.auth.revokeAllSessions(uid, sessionId) : Promise.resolve(), events.log({
    targetUid: uid,
    type: 'email-change',
    email,
    newEmail: ''
...
UserEmail.isValidationPending = async (uid, email) => {
  ...
  return !!(confirmObj && (!email || email === confirmObj.email) && Date.now() < parseInt(confirmObj.expires, 10));
};
...
UserEmail.sendValidationEmail = async function (uid, options) {
  ...
  if (plugins.hooks.hasListeners('action:user.verify')) {
    plugins.hooks.fire('action:user.verify', {
      uid: uid,
      data: data
    });
  } else {
...
UserEmail.confirmByCode = async function (code, sessionId) {
  ...
  await Promise.all([UserEmail.confirmByUid(confirmObj.uid), db.delete(`confirm:${code}`), events.log({
    type: 'email-change',
    oldEmail,
...
UserEmail.confirmByUid = async function (uid, callerUid = 0) {
  ...
  if (confirmedEmails.length) {
    ...
    await db.sortedSetRemoveBulk(confirmedEmails.map(email => [`email:sorted`, `${email.toLowerCase()}:${uid}`]));
  }
  await Promise.all([db.sortedSetAddBulk([['email:uid', uid, currentEmail.toLowerCase()], ['email:sorted', 0, `${currentEmail.toLowerCase()}:${uid}`], [`user:${uid}:emails`, Date.now(), `${currentEmail}:${Date.now()}:${callerUid}`]]), user.setUserField(uid, 'email:confirmed', 1), groups.join('verified-users', uid), groups.leave('unverified-users', uid), user.email.expireValidation(uid), user.reset.cleanByUid(uid)]);
