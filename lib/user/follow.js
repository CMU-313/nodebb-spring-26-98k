...
module.exports = function (User) {
  ...
  async function toggleFollow(type, uid, theiruid) {
    ...
    const [exists, isFollowing] = await Promise.all([User.exists(theiruid), User.isFollowing(uid, theiruid)]);
    if (!exists) {
    ...
    if (type === 'follow') {
      ...
      const now = Date.now();
      await db.sortedSetAddBulk([[`following:${uid}`, now, theiruid], [`followers:${theiruid}`, now, uid]]);
    } else {
      if (!isFollowing) {
        throw new Error('[[error:not-following]]');
      }
      await db.sortedSetRemoveBulk([[`following:${uid}`, theiruid], [`followers:${theiruid}`, uid]]);
    }
    const [followingCount, followingRemoteCount, followerCount, followerRemoteCount] = await db.sortedSetsCard([`following:${uid}`, `followingRemote:${uid}`, `followers:${theiruid}`, `followersRemote:${theiruid}`]);
    await Promise.all([User.setUserField(uid, 'followingCount', followingCount + followingRemoteCount), User.setUserField(theiruid, 'followerCount', followerCount + followerRemoteCount)]);
  }
  ...
  async function getFollow(uid, type, start, stop) {
    ...
    let uids = await db.getSortedSetRevRange([`${type}:${uid}`, `${type}Remote:${uid}`], start, stop);
