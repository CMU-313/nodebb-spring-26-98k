...
User.exists = async function (uids) {
  ...
  const [localExists, remoteExists] = await Promise.all([db.isSortedSetMembers('users:joindate', uids), meta.config.activitypubEnabled ? db.exists(uids.map(uid => `userRemote:${uid}`)) : uids.map(() => false)]);
...
User.getUsers = async function (uids, uid) {
  const userData = await User.getUsersWithFields(uids, ['uid', 'username', 'userslug', 'picture', 'status', 'postcount', 'reputation', 'email:confirmed', 'lastonline', 'flags', 'banned', 'banned:expire', 'joindate'], uid);
...
User.getUidsByUserslugs = async function (userslugs) {
  ...
  async function getApSlugs() {
    ...
    const apUids = await db.getObjectFields('handle:uid', apSlugs.map(slug => String(slug).toLowerCase()));
    return apUids;
  }
  const [apUids, normalUids] = await Promise.all([apSlugs.length ? getApSlugs() : [], normalSlugs.length ? db.sortedSetScores('userslug:uid', normalSlugs) : []]);
...
User.getAdminsandGlobalModsandModerators = async function () {
  const results = await Promise.all([groups.getMembers('administrators', 0, -1), groups.getMembers('Global Moderators', 0, -1), User.getModeratorUids()]);
  return await User.getUsersData(_.union(...results));
};
...
User.addInterstitials = function (callback) {
  plugins.hooks.register('core', {
    ...
    method: [User.interstitials.email, User.interstitials.gdpr, User.interstitials.tou]
  });
  callback();
};
