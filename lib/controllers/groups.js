...
groupsController.list = async function (req, res) {
  ...
  const [allowGroupCreation, [groupData, pageCount]] = await Promise.all([privileges.global.can('group:create', req.uid), getGroups(req, sort, page)]);
  res.locals.linkTags = [{
    rel: 'canonical',
...
async function getGroups(req, sort, page) {
  ...
  const [groupData, groupCount] = await Promise.all([groups.getGroupsBySort(sort, start, stop), groups.getGroupCountBySort(sort)]);
...
groupsController.details = async function (req, res, next) {
  ...
  const [exists, isHidden, isAdmin, isGlobalMod] = await Promise.all([groups.exists(groupName), groups.isHidden(groupName), privileges.admin.can('admin:groups', req.uid), user.isGlobalModerator(req.uid)]);
  if (!exists) {
    return next();
  }
  ...
  const [groupData, posts] = await Promise.all([groups.get(groupName, {
    uid: req.uid,
