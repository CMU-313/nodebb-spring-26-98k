...
settingsController.get = async function (req, res, next) {
  ...
  const [notificationSettings, routes, bsSkinOptions] = await Promise.all([getNotificationSettings(userData), getHomePageRoutes(userData), getSkinOptions(userData), getChatAllowDenyList(userData)]);
...
async function getSkinOptions(userData) {
  ...
  bootswatchSkinOptions.push(...meta.css.supportedSkins.map(skin => ({
...
async function getChatAllowDenyList(userData) {
  const [chatAllowListUsers, chatDenyListUsers] = await Promise.all([user.getUsersFields(userData.settings.chatAllowList, ['uid', 'username', 'picture']), user.getUsersFields(userData.settings.chatDenyList, ['uid', 'username', 'picture'])]);
