  ...
  'account/watched': {
    ...
    getTopics: async function (set, req, start, stop) {
      ...
      tids = tids.map((tid, i) => ({
        tid: tid,
        score: scores[i]
      })).sort((a, b) => b.score - a.score).slice(start, stop + 1).map(t => t.tid);
...
async function getPostsFromUserSet(template, req, res) {
  ...
  const sets = await data.getSets(req.uid, {
    uid,
    username,
    userslug
  });
  ...
  payload.sortOptions = [{
    url: `${baseUrl}?sort=votes`,
    name: '[[global:votes]]'
  }, {
    url: `${baseUrl}?sort=posts`,
    name: '[[global:posts]]'
  }, {
    url: `${baseUrl}?sort=views`,
    name: '[[global:views]]'
  }, {
    url: `${baseUrl}?sort=lastpost`,
    name: '[[global:lastpost]]'
  }, {
    url: `${baseUrl}?sort=firstpost`,
    name: '[[global:firstpost]]'
  }];
