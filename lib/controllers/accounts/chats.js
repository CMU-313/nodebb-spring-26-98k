...
chatsController.get = async function (req, res, next) {
  ...
  if (!isSwitch) {
    const [recentChats, publicRooms, privateRoomCount] = await Promise.all([messaging.getRecentChats(req.uid, uid, 0, 29), messaging.getPublicRooms(req.uid, uid), db.sortedSetCard(`uid:${uid}:chat:rooms`)]);
    if (!recentChats) {
      return next();
    }
  ...
  const [canViewInfo, canUploadImage, canUploadFile] = await privileges.global.can(['view:users:info', 'upload:post:image', 'upload:post:file'], uid);
...
chatsController.redirectToMessage = async function (req, res, next) {
  ...
  const [userslug, roomId] = await Promise.all([user.getUserField(req.uid, 'userslug'), messaging.getMessageField(mid, 'roomId')]);
  if (!userslug || !roomId) {
    return next();
  }
