...
editController.get = async function (req, res, next) {
  ...
  const [canUseSignature, canManageUsers, customUserFields] = await Promise.all([privileges.global.can('signature', req.uid), privileges.admin.can('admin:users', req.uid), accountHelpers.getCustomUserFields(req.uid, userData)]);
  ...
  if (req.uid === res.locals.uid || canManageUsers) {
    const {
      associations
    } = await plugins.hooks.fire('filter:auth.list', {
      uid: res.locals.uid,
      associations: []
    });
...
async function renderRoute(name, req, res) {
  ...
  const [isAdmin, {
    username,
    userslug
  }, hasPassword] = await Promise.all([privileges.admin.can('admin:users', req.uid), user.getUserFields(res.locals.uid, ['username', 'userslug']), user.hasPassword(res.locals.uid)]);
