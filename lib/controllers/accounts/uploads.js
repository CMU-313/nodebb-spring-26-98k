...
uploadsController.get = async function (req, res) {
  ...
  const [itemCount, uploadNames] = await Promise.all([db.sortedSetCard(`uid:${res.locals.uid}:uploads`), db.getSortedSetRevRange(`uid:${res.locals.uid}:uploads`, start, stop)]);
