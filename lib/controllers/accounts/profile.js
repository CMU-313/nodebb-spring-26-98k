...
profileController.get = async function (req, res, next) {
  ...
  const [latestPosts, bestPosts, customUserFields] = await Promise.all([getLatestPosts(req.uid, userData), getBestPosts(req.uid, userData), accountHelpers.getCustomUserFields(req.uid, userData), posts.parseSignature(userData, req.uid)]);
  userData.customUserFields = customUserFields;
  userData.posts = latestPosts;
...
async function getPosts(callerUid, userData, setSuffix) {
  ...
  do {
    ...
    if (pids.length) {
      ({
        pids
      } = await plugins.hooks.fire('filter:account.profile.getPids', {
        uid: callerUid,
        userData,
        setSuffix,
        pids
      ...
      postData.push(...p.filter(p => p && p.topic && (isAdmin || isModOfCid[p.topic.cid] || p.topic.scheduled && cidToCanSchedule[p.topic.cid] || !p.deleted && !p.topic.deleted)));
    }
    start += count;
