...
helpers.getUserDataByUserSlug = async function (userslug, callerUID, query = {}) {
  ...
  const [results, canFlag, flagged, flagId] = await Promise.all([getAllData(uid, callerUID), privileges.users.canFlag(callerUID, uid), flags.exists('user', uid, callerUID), flags.getFlagIdByTarget('user', uid)]);
  if (!results.userData) {
...
helpers.getCustomUserFields = async function (callerUID, userData) {
  ...
  const [isAdmin, isModOfAny] = await Promise.all([privileges.users.isAdministrator(callerUID), user.isModeratorOfAnyCategory(callerUID)]);
  const fields = allFields.filter(field => {
    const visibility = field.visibility || 'all';
    const visibilityCheck = isAdmin || isModOfAny || isSelf || visibility === 'all' || visibility === 'loggedin' && String(callerUID) !== '0' && String(callerUID) !== '-1';
...
async function getAllData(uid, callerUID) {
  const [[isTargetAdmin, isCallerAdmin], isGlobalModerator] = await Promise.all([user.isAdministrator([uid, callerUID]), user.isGlobalModerator(callerUID)]);
...
async function getCounts(userData, callerUID) {
  ...
  userData.blocksCount = counts.blocks || 0;
