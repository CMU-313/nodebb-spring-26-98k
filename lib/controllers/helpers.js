...
helpers.notAllowed = async function (req, res, error) {
  ({
    error
  } = await plugins.hooks.fire('filter:helpers.notAllowed', {
    req,
    res,
    error
  }));
...
helpers.redirect = function (res, url, permanent) {
  if (url.hasOwnProperty('external')) {
    ...
    if (res.locals.isAPI) {
      res.set('X-Redirect', encodeURIComponent(redirectUrl)).status(200).json({
...
async function getCategoryData(cids, uid, selectedCid, states, privilege) {
  const [visibleCategories, selectData] = await Promise.all([helpers.getVisibleCategories({
    cids,
    uid,
    states,
    privilege,
