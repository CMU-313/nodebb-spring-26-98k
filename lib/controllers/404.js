...
exports.handle404 = helpers.try(async (req, res) => {
  ...
  } else if (activitypub.helpers.assertAccept(req.headers.accept) || req.headers['Content-Type'] && activitypub._constants.acceptableTypes.includes(req.headers['Content-Type'])) {
    res.sendStatus(404);
  } else if (!res.locals.isAPI && (req.path.startsWith(`${relativePath}/assets/uploads`) || req.get('accept') && !req.get('accept').includes('text/html') || req.path === '/favicon.ico')) {
...
exports.send404 = helpers.try(async (req, res) => {
  ...
  const icons = ['fa-hippo', 'fa-cat', 'fa-otter', 'fa-dog', 'fa-cow', 'fa-fish', 'fa-dragon', 'fa-horse', 'fa-dove'];
