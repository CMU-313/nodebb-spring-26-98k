...
async function getUsers(req, res) {
  ...
  const [count, users, customUserFields] = await Promise.all([getCount(set), loadUserInfo(req.uid, uids), getCustomUserFields()]);
...
async function loadUserInfo(callerUid, uids) {
  ...
  const [isAdmin, userData, lastonline, confirmObjs, ips] = await Promise.all([user.isAdministrator(uids), user.getUsersWithFields(uids, userFields, callerUid), db.sortedSetScores('users:online', uids), getConfirmObjs(), getIPs()]);
...
usersController.customFields = async function (req, res) {
  ...
  res.render('admin/manage/users/custom-fields', {
    fields: fields
  });
};