...
groupsController.get = async function (req, res, next) {
  ...
  const [groupNames, group] = await Promise.all([getGroupNames(), groups.get(groupName, {
    uid: req.uid,
...
async function getGroupNames() {
  ...
  groupEntries = groupEntries.map(g => ({
    slug: g[0],
    name: g[1]
  }));
  return groupEntries.filter(g => g.name !== 'registered-users' && g.name !== 'verified-users' && g.name !== 'unverified-users' && g.name !== groups.BANNED_USERS).sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
}
