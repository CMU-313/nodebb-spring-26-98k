...
settingsController.user = async (req, res) => {
  const [notificationTypes, groupData] = await Promise.all([notifications.getAllNotificationTypes(), groups.getNonPrivilegeGroups('groups:createtime', 0, -1)]);
...
settingsController.api = async (req, res) => {
  ...
  const [tokens, count] = await Promise.all([api.utils.tokens.list(start, stop), api.utils.tokens.count()]);
...
settingsController.activitypub = async (req, res) => {
  const [instanceCount, rules, relays] = await Promise.all([activitypub.instances.getCount(), activitypub.rules.list(), activitypub.relays.list()]);
