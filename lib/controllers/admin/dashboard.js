...
async function getStats() {
  ...
  let results = await Promise.all([getStatsFromAnalytics('uniquevisitors', ''), getStatsFromAnalytics('logins', 'loginCount'), getStatsForSet('users:joindate', 'userCount'), getStatsForSet('posts:pid', 'postCount'), getStatsForSet('topics:tid', 'topicCount')]);
  ...
  ({
    results
  } = await plugins.hooks.fire('filter:admin.getStats', {
    results,
    helpers: {
      getStatsForSet,
...
dashboardController.getLogins = async (req, res) => {
  ...
  stats = stats.filter(stat => stat.name === '[[admin/dashboard:logins]]').map(({
    ...stat
  }) => {
    delete stat.href;
    return stat;
...
dashboardController.getUsers = async (req, res) => {
  let stats = await getStats();
  stats = stats.filter(stat => stat.name === '[[admin/dashboard:new-users]]').map(({
    ...stat
  }) => {
    delete stat.href;
    return stat;
...
dashboardController.getTopics = async (req, res) => {
  let stats = await getStats();
  stats = stats.filter(stat => stat.name === '[[admin/dashboard:topics]]').map(({
    ...stat
  }) => {
    delete stat.href;
    return stat;
...
dashboardController.getSearches = async (req, res) => {
  ...
  if (start && end && start <= end) {
    ...
    const daysData = await Promise.all(daysArr.map(async d => db.getSortedSetRevRangeWithScores(`searches:${d.getTime()}`, 0, -1)));
    const map = {};
    daysData.forEach(d => {
      d.forEach(search => {
        if (!map[search.value]) {
    ...
    searches = Object.keys(map).map(key => ({
      value: key,
      score: map[key]
