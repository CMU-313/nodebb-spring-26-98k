...
topicsController.get = async function getTopic(req, res, next) {
  ...
  if (req.params.post_index && !utils.isNumber(req.params.post_index) && req.params.post_index !== 'unread' || !utils.isNumber(tid) && !validator.isUUID(String(tid))) {
    return next();
  }
  ...
  const [userPrivileges, settings, rssToken] = await Promise.all([privileges.topics.get(tid, req.uid), user.getSettings(req.uid), user.auth.getFeedToken(req.uid)]);
  ...
  if (userPrivileges.disabled || invalidPagination || topicData.scheduled && !userPrivileges.view_scheduled) {
    return next();
  }
...
topicsController.pagination = async function (req, res, next) {
  ...
  const [userPrivileges, settings] = await Promise.all([privileges.topics.get(tid, req.uid), user.getSettings(req.uid)]);
