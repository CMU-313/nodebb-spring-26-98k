...
Users.get = async (req, res) => {
  helpers.formatApiResponse(200, res, await api.users.get(req, {
    ...req.params
  }));
};
...
Users.changePicture = async (req, res) => {
  await api.users.changePicture(req, {
    ...req.body,
    uid: req.params.uid
  });
  helpers.formatApiResponse(200, res);
...
Users.getStatus = async (req, res) => {
  helpers.formatApiResponse(200, res, await api.users.getStatus(req, {
    ...req.params
  }));
};
Users.checkStatus = async (req, res) => {
  ...
  const {
    status: current
  } = await api.users.getStatus(req, {
    uid
  });
  helpers.formatApiResponse(current === status ? 200 : 404, res);
};
Users.getPrivateRoomId = async (req, res) => {
  helpers.formatApiResponse(200, res, await api.users.getPrivateRoomId(req, {
    ...req.params
  }));
};
...
Users.changePassword = async (req, res) => {
  await api.users.changePassword(req, {
    ...req.body,
    uid: req.params.uid
  });
  helpers.formatApiResponse(200, res);
...
Users.deleteToken = async (req, res) => {
  const ok = await api.users.deleteToken(req, {
    ...req.params
  });
  helpers.formatApiResponse(ok ? 200 : 404, res);
};
Users.revokeSession = async (req, res) => {
  await api.users.revokeSession(req, {
    ...req.params
  });
  helpers.formatApiResponse(200, res);
};
...
Users.getInviteGroups = async function (req, res) {
  return helpers.formatApiResponse(200, res, await api.users.getInviteGroups(req, {
    ...req.params
  }));
};
...
Users.listEmails = async (req, res) => {
  const emails = await api.users.listEmails(req, {
    ...req.params
  });
  if (emails) {
    helpers.formatApiResponse(200, res, {
...
Users.getEmail = async (req, res) => {
  const ok = await api.users.getEmail(req, {
    ...req.params
  });
...
Users.checkExportByType = async (req, res) => {
  const stat = await api.users.checkExportByType(req, {
    ...req.params
  });
...
Users.getExportByType = async (req, res, next) => {
  const data = await api.users.getExportByType(req, {
    ...req.params
  });
  if (!data) {
    return next();
