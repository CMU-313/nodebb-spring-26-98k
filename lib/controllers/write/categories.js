...
Categories.delete = async (req, res) => {
  await api.categories.delete(req, {
    cid: req.params.cid
  });
  helpers.formatApiResponse(200, res);
};
Categories.getTopicCount = async (req, res) => {
  helpers.formatApiResponse(200, res, await api.categories.getTopicCount(req, {
    ...req.params
  }));
};
Categories.getPosts = async (req, res) => {
  const posts = await api.categories.getPosts(req, {
    ...req.params
  });
  helpers.formatApiResponse(200, res, {
    posts
...
Categories.getChildren = async (req, res) => {
  ...
  helpers.formatApiResponse(200, res, await api.categories.getChildren(req, {
    cid,
    start
  }));
};
Categories.getTopics = async (req, res) => {
  ...
  const result = await api.categories.getTopics(req, {
    ...req.query,
    cid
  });
...
Categories.setWatchState = async (req, res) => {
  ...
  const {
    cids: modified
  } = await api.categories.setWatchState(req, {
    cid,
    state,
    uid
  });
