...
async function registerAndLoginUser(req, res, userData) {
  ...
  if (deferRegistration) {
    ...
    res.json({
      next: `${nconf.get('relative_path')}/register/complete`
    });
    return;
  }
  ...
  try {
    ...
    const callbacks = data.interstitials.reduce((memo, cur) => {
      if (cur.hasOwnProperty('callback') && typeof cur.callback === 'function') {
        ...
        if (cur.callback.constructor && cur.callback.constructor.name === 'AsyncFunction' || cur.callback.length === 2) {
...
authenticationController.registerAbort = async (req, res) => {
  if (req.uid && req.session.registration) {
    ...
    const {
      interstitials
    } = await user.interstitials.get(req, req.session.registration);
...
authenticationController.login = async (req, res, next) => {
  let {
    strategy
  } = await plugins.hooks.fire('filter:login.override', {
    req,
    strategy: 'local'
  });
...
authenticationController.onSuccessfulLogin = async function (req, uid, trackSession = true) {
  ...
  try {
    ...
    }), trackSession ? user.auth.addSession(uid, req.sessionID) : undefined, user.updateLastOnlineTime(uid), user.onUserOnline(uid, Date.now()), analytics.increment('logins'), db.incrObjectFieldBy('global', 'loginCount', 1)]);
    sockets.in(`sess_${req.sessionID}`).emit('checkSession', uid);
    plugins.hooks.fire('action:user.loggedIn', {
      uid: uid,
      req: req
    });
  } catch (err) {
