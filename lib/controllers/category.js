...
categoryController.get = async function (req, res, next) {
  ...
  const [categoryFields, userPrivileges, tagData, userSettings, rssToken] = await Promise.all([categories.getCategoryFields(cid, ['slug', 'disabled', 'link']), privileges.categories.get(cid, req.uid), helpers.getSelectedTag(req.query.tag), user.getSettings(req.uid), user.auth.getFeedToken(req.uid)]);
  if (!categoryFields.slug || categoryFields && categoryFields.disabled || userSettings.usePagination && currentPage < 1) {
    return next();
  }
  if (topicIndex < 0) {
  ...
  await Promise.all([buildBreadcrumbs(req, categoryData), categories.setUnread([categoryData], allCategories.map(c => c.cid).concat(cid), req.uid)]);
