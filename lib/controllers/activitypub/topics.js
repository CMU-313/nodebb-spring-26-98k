...
const validSorts = ['recently_replied', 'recently_created', 'most_posts', 'most_votes', 'most_views'];
controller.list = async function (req, res) {
  ...
  const [userPrivileges, tagData, userSettings, rssToken] = await Promise.all([privileges.categories.get('-1', req.uid), helpers.getSelectedTag(req.query.tag), user.getSettings(req.uid), user.auth.getFeedToken(req.uid)]);
  ...
  tids = await categories.sortTidsBySet(tids, sort);
  data.topicCount = tids.length;
  data.topics = await topics.getTopicsByTids(tids, {
    uid: req.uid
  });
  topics.calculateTopicIndices(data.topics, start);
  let cids = await user.getCategoriesByStates(req.uid, [categories.watchStates.tracking, categories.watchStates.watching]);
  ...
  await Promise.all([categories.getRecentTopicReplies(categoryData, req.uid, req.query), categories.setUnread(data.categories, cids, req.uid)]);
