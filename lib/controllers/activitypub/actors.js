...
Actors.note = async function (req, res, next) {
  ...
  const {
    to,
    cc
  } = await activitypub.buildRecipients(payload, {
    pid: post.pid,
    uid: post.user.uid
  });
  payload.to = to;
...
Actors.topic = async function (req, res, next) {
  ...
  try {
    ...
    let pids;
    try {
      [collection, pids] = await Promise.all([activitypub.helpers.generateCollection({
...
Actors.category = async function (req, res, next) {
  const [exists, allowed] = await Promise.all([categories.exists(req.params.cid), privileges.categories.can('find', req.params.cid, activitypub._constants.uid)]);
  if (!exists || !allowed) {
    return next('route');
  }
...
Actors.message = async function (req, res) {
  ...
  const payload = await activitypub.mocks.notes.private({
    messageObj
  });
  res.status(200).json(payload);
};