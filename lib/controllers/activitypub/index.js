  ...
  try {
    ...
    if (typeof result === 'string') {
      ...
      const {
        id,
        type
      } = await activitypub.get('uid', req.uid, url.href);
      switch (true) {
...
Controller.getFollowing = async (req, res) => {
  ...
  } = await user.getUserFields(req.params.uid, ['followingCount', 'followingRemoteCount']);
  ...
  if (collection.hasOwnProperty('orderedItems')) {
    collection.orderedItems = collection.orderedItems.map(({
      uid
    }) => {
...
Controller.getFollowers = async (req, res) => {
  const {
    followerCount,
    followerRemoteCount
  } = await user.getUserFields(req.params.uid, ['followerCount', 'followerRemoteCount']);
