...
recentController.getData = async function (req, url, sort, selectedTerm = 'alltime') {
  ...
  term = term || 'alltime';
  const [settings, categoryData, tagData, rssToken, canPost, isPrivileged] = await Promise.all([user.getSettings(req.uid), helpers.getSelectedCategory(cid), helpers.getSelectedTag(tag), user.auth.getFeedToken(req.uid), privileges.categories.canPostTopic(req.uid), user.isPrivileged(req.uid)]);
