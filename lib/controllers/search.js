...
searchController.search = async function (req, res, next) {
  ...
  let allowed = req.query.in === 'users' && userPrivileges['search:users'] || req.query.in === 'tags' && userPrivileges['search:tags'] || req.query.in === 'categories' || ['titles', 'titlesposts', 'posts', 'bookmarks'].includes(req.query.in) && userPrivileges['search:content'];
  ({
    allowed
  } = await plugins.hooks.fire('filter:search.isAllowed', {
    uid: req.uid,
    query: req.query,
    allowed
  }));
  ...
  searchData.filters = {
    ...
    users: {
      ...
      label: translator.compile('search:posted-by-usernames', (Array.isArray(data.postedBy) ? data.postedBy : []).map(u => validator.escape(String(u))).join(', '))
    },
    tags: {
      active: !!(Array.isArray(data.hasTags) && data.hasTags.length),
      label: translator.compile('search:tags-x', (Array.isArray(data.hasTags) ? data.hasTags : []).map(u => validator.escape(String(u))).join(', '))
    },
    categories: {
      active: !!(Array.isArray(data.categories) && data.categories.length && (data.categories.length > 1 || data.categories[0] !== 'all')),
