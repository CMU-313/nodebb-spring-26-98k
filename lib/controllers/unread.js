...
unreadController.get = async function (req, res) {
  ...
  const [categoryData, tagData, userSettings, canPost, isPrivileged] = await Promise.all([helpers.getSelectedCategory(cid), helpers.getSelectedTag(tag), user.getSettings(req.uid), privileges.categories.canPostTopic(req.uid), user.isPrivileged(req.uid)]);
