...
Controller.generate = async (req, res) => {
  ...
  let swContents = await readFile(swPath, {
    encoding: 'utf-8'
  });
  res.status(200).type('application/javascript').set('Service-Worker-Allowed', `${nconf.get('relative_path')}/`);
  let scripts = new Set();
  ({
    scripts
  } = await plugins.hooks.fire('filter:service-worker.scripts', {
    scripts
  }));
  if (!scripts.size) {
    res.sendFile(swPath);
  } else {
    const urls = await Promise.all(Array.from(scripts).map(async pathname => {
      try {
