...
Notifications.baseTypes = ['notificationType_upvote', 'notificationType_new-topic', 'notificationType_new-topic-with-tag', 'notificationType_new-topic-in-category', 'notificationType_new-reply', 'notificationType_post-edit', 'notificationType_follow', 'notificationType_new-chat', 'notificationType_new-group-chat', 'notificationType_new-public-chat', 'notificationType_group-invite', 'notificationType_group-leave', 'notificationType_group-request-membership', 'notificationType_new-reward'];
Notifications.privilegedTypes = ['notificationType_new-register', 'notificationType_post-queue', 'notificationType_new-post-flag', 'notificationType_new-user-flag'];
const notificationPruneCutoff = 2592000000;
...
Notifications.getMultiple = async function (nids) {
  ...
  let [usersData, categoriesData] = await Promise.all([User.getUsersFields(userKeys, ['username', 'userslug', 'picture']), categories.getCategoriesFields(userKeys, ['cid', 'name', 'slug', 'picture'])]);
    ...
    if (notification) {
      intFields.forEach(field => {
        if (notification.hasOwnProperty(field)) {
          notification[field] = utils.isNumber(notification[field]) ? parseInt(notification[field], 10) || 0 : notification[field];
        }
      });
...
Notifications.create = async function (data) {
  ...
  await Promise.all([db.sortedSetAdd('notifications', now, data.nid), db.setObject(`notifications:${data.nid}`, data)]);
  return data;
};
...
async function pushToUids(uids, notification) {
  async function sendNotification(uids) {
    ...
    await Promise.all([db.sortedSetsAdd(unreadKeys, notification.datetime, notification.nid), db.sortedSetsRemove(readKeys, notification.nid)]);
...
Notifications.rescind = async function (nids) {
  ...
  await plugins.hooks.fire('static:notifications.rescind', {
    nids
  });
  await Promise.all([db.sortedSetRemove('notifications', nids), db.deleteAll(nids.map(nid => `notifications:${nid}`))]);
  plugins.hooks.fire('action:notifications.rescind', {
    nids
  });
};
...
Notifications.markUnread = async function (nid, uid) {
  ...
  await Promise.all([db.sortedSetRemove(`uid:${uid}:notifications:read`, nid), db.sortedSetAdd(`uid:${uid}:notifications:unread`, notification.datetime, nid)]);
};
Notifications.markReadMultiple = async function (nids, uid) {
  ...
  await Promise.all([db.sortedSetRemove(`uid:${uid}:notifications:unread`, nids), db.sortedSetAdd(`uid:${uid}:notifications:read`, datetimes, nids)]);
};
...
Notifications.merge = async function (notifications) {
  const mergeIds = ['notifications:upvoted-your-post-in', 'notifications:user-started-following-you', 'notifications:user-posted-to', 'notifications:user-flagged-post-in', 'notifications:user-flagged-user', 'new-chat', 'notifications:user-posted-in-public-room', 'new-register', 'post-queue', 'notifications:activitypub.announce'];
        ...
        case 'new-chat':
          {
            ...
            notifObj.bodyShort = isGroupChat || roomName !== `[[modules:chat.room-id, ${roomId}]]` ? `[[notifications:new-messages-in, ${set.length}, ${roomName}]]` : `[[notifications:new-messages-from, ${set.length}, ${user.displayname}]]`;
            break;
          }
