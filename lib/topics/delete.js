...
module.exports = function (Topics) {
  Topics.delete = async function (tid, uid) {
    ...
    await Promise.all([db.sortedSetRemove(`cid:${cid}:pids`, pids), resolveTopicPostFlags(pids, uid), Topics.setTopicFields(tid, {
      deleted: 1,
      deleterUid: uid,
  ...
  Topics.restore = async function (tid) {
    ...
    await Promise.all([Topics.deleteTopicFields(tid, ['deleterUid', 'deletedTimestamp']), addTopicPidsToCid(tid, cid)]);
  ...
  Topics.purgePostsAndTopic = async function (tid, uid) {
    ...
    await batch.processSortedSet(`tid:${tid}:posts`, async pids => {
      ...
      await db.sortedSetRemove(`tid:${tid}:posts`, pids);
    }, {
      alwaysStartAt: 0,
      batch: 500
    });
  ...
  Topics.purge = async function (tid, uid) {
    ...
    await Promise.all([db.deleteAll([`tid:${tid}:followers`, `tid:${tid}:ignorers`, `tid:${tid}:posts`, `tid:${tid}:posts:votes`, `tid:${tid}:bookmarks`, `tid:${tid}:posters`]), db.sortedSetsRemove(['topics:tid', 'topics:recent', 'topics:scheduled'], tid), db.sortedSetsRemove(['views', 'posts', 'votes'].map(prop => `${utils.isNumber(tid) ? 'topics' : 'topicsRemote'}:${prop}`), tid), deleteTopicFromCategoryAndUser(tid), Topics.deleteTopicTags(tid), Topics.events.purge(tid), Topics.thumbs.deleteAll(tid), Topics.crossposts.removeAll(tid), reduceCounters(tid)]);
  ...
  async function deleteFromFollowersIgnorers(tid) {
    const [followers, ignorers] = await Promise.all([db.getSetMembers(`tid:${tid}:followers`), db.getSetMembers(`tid:${tid}:ignorers`)]);
  ...
  async function deleteTopicFromCategoryAndUser(tid) {
    ...
    await Promise.all([db.sortedSetsRemove([`cid:${topicData.cid}:tids`, `cid:${topicData.cid}:tids:pinned`, `cid:${topicData.cid}:tids:create`, `cid:${topicData.cid}:tids:posts`, `cid:${topicData.cid}:tids:lastposttime`, `cid:${topicData.cid}:tids:votes`, `cid:${topicData.cid}:tids:views`, `cid:${topicData.cid}:recent_tids`, `cid:${topicData.cid}:uid:${topicData.uid}:tids`, `uid:${topicData.uid}:topics`], tid), user.decrementUserFieldBy(topicData.uid, 'topiccount', 1)]);
  ...
  async function reduceCounters(tid) {
    ...
    await Promise.all([db.incrObjectFieldBy('global', 'postCount', postCountChange), db.incrObjectFieldBy(`${utils.isNumber(topicData.cid) ? 'category' : 'categoryRemote'}:${topicData.cid}`, 'post_count', postCountChange), db.incrObjectFieldBy(`${utils.isNumber(topicData.cid) ? 'category' : 'categoryRemote'}:${topicData.cid}`, 'topic_count', incr)]);
  }
};