...
Topics.getTopicsByTids = async function (tids, options) {
  ...
  async function loadTopics() {
    ...
    const uids = _.uniq(topics.map(t => t && t.uid && t.uid.toString()).filter(v => utils.isNumber(v) || activitypub.helpers.isUri(v)));
    ...
    const [teasers, users, userSettings, categoriesData, guestHandles, thumbs] = await Promise.all([Topics.getTeasers(topics, options), user.getUsersFields(uids, ['uid', 'username', 'fullname', 'userslug', 'reputation', 'postcount', 'picture', 'signature', 'banned', 'status']), loadShowfullnameSettings(), categories.getCategoriesFields(cids, ['cid', 'name', 'slug', 'icon', 'backgroundImage', 'imageClass', 'bgColor', 'color', 'disabled']), loadGuestHandles(), Topics.thumbs.load(topics)]);
  ...
  const [result, hasRead, followData, bookmarks, callerSettings] = await Promise.all([loadTopics(), Topics.hasReadTopics(tids, uid), Topics.getFollowData(tids, uid), Topics.getUserBookmarks(tids, uid), user.getSettings(uid)]);
  ...
  result.topics.forEach((topic, i) => {
    if (topic) {
      ...
      topic.bookmark = bookmarks[i] && (sortNewToOld ? Math.max(1, topic.postcount + 2 - bookmarks[i]) : Math.min(topic.postcount, bookmarks[i] + 1));
...
Topics.getTopicWithPosts = async function (topicData, set, uid, start, stop, reverse) {
  const [posts, category, tagWhitelist, threadTools, followData, bookmark, postSharing, deleter, merger, forker, related, thumbs, events] = await Promise.all([Topics.getTopicPosts(topicData, set, start, stop, uid, reverse), categories.getCategoryData(topicData.cid), categories.getTagWhitelist([topicData.cid]), plugins.hooks.fire('filter:topic.thread_tools', {
    topic: topicData,
    uid: uid,
    tools: []
  }), Topics.getFollowData([topicData.tid], uid), Topics.getUserBookmark(topicData.tid, uid), social.getActivePostSharing(), getDeleter(topicData), getMerger(topicData), getForker(topicData), Topics.getRelatedTopics(topicData, uid), Topics.thumbs.load([topicData]), Topics.events.get(topicData.tid, uid, reverse)]);
...
async function getMerger(topicData) {
  ...
  const [merger, mergedIntoTitle] = await Promise.all([user.getUserFields(topicData.mergerUid, ['username', 'userslug', 'picture']), Topics.getTopicField(topicData.mergeIntoTid, 'title')]);
...
async function getForker(topicData) {
  ...
  const [forker, forkedFromTitle] = await Promise.all([user.getUserFields(topicData.forkerUid, ['username', 'userslug', 'picture']), Topics.getTopicField(topicData.forkedFromTid, 'title')]);
