...
module.exports = function (Topics) {
  ...
  async function getTids(params) {
    ...
    const unreadFollowed = await db.isSortedSetMembers(`uid:${params.uid}:followed_tids`, tids_unread.map(t => t.value));
    tids_unread.forEach((t, i) => {
      isTopicsFollowed[t.value] = unreadFollowed[i];
    });
    const unreadTopics = _.unionWith(categoryTids, followedTids, (a, b) => a.value === b.value).filter(t => !ignoredTids.includes(t.value) && (!userReadTimes[t.value] || t.score > userReadTimes[t.value])).concat(tids_unread.filter(t => !ignoredTids.includes(t.value))).sort((a, b) => b.score - a.score);
    ...
    const topicData = (await Topics.getTopicsFields(tids, ['tid', 'cid', 'uid', 'postcount', 'deleted', 'scheduled', 'tags'])).filter(t => t.scheduled || !t.deleted);
    ...
    topicData.forEach(topic => {
      if (topic && topic.cid && (!filterCids || filterCids.includes(topic.cid)) && (!filterTags || filterTags.every(tag => topic.tags.find(topicTag => topicTag.value === tag))) && !blockedUids.includes(topic.uid)) {
        if (isTopicsFollowed[topic.tid] || [categories.watchStates.watching, categories.watchStates.tracking].includes(userCidState[topic.cid])) {
  ...
  async function getWatchedTrackedCids(uid) {
    ...
    const cids = await user.getCategoriesByStates(uid, [categories.watchStates.watching, categories.watchStates.tracking]);
  ...
  async function getFollowedTids(params) {
    const keys = params.cid ? params.cid.map(cid => `cid:${cid}:tids:lastposttime`) : 'topics:recent';
  ...
  Topics.pushUnreadCount = async function (uid) {
    ...
    const results = await Topics.getUnreadTids({
      uid: uid,
      count: true
    });
  ...
  Topics.markAsRead = async function (tids, uid) {
    ...
    const now = Date.now();
    const topics = topicScores.filter((t, i) => t.lastposttime && (!userScores[i] || userScores[i] < t.lastposttime || userScores[i] > now));
    tids = topics.map(t => t.tid);
    if (!tids.length) {
      return false;
    }
    const scores = topics.map(topic => topic.scheduled ? topic.lastposttime : now);
    await Promise.all([db.sortedSetAdd(`uid:${uid}:tids_read`, scores, tids), db.sortedSetRemove(`uid:${uid}:tids_unread`, tids)]);
  ...
  Topics.markCategoryUnreadForAll = async function () {
  ...
  Topics.hasReadTopics = async function (tids, uid) {
    ...
    const [topicScores, remoteTopicScores, userScores, tids_unread, blockedUids] = await Promise.all([db.sortedSetScores('topics:recent', tids), getRemoteTopicScores(tids), db.sortedSetScores(`uid:${uid}:tids_read`, tids), db.sortedSetScores(`uid:${uid}:tids_unread`, tids), user.blocks.list(uid)]);
    const cutoff = await Topics.unreadCutoff(uid);
    const result = tids.map((tid, index) => {
      const read = !tids_unread[index] && ((topicScores[index] || remoteTopicScores[index]) < cutoff || !!(userScores[index] && userScores[index] >= topicScores[index]));
      return {
        tid: tid,
        read: read,
        index: index
  ...
  Topics.markUnread = async function (tid, uid) {
    ...
    await Promise.all([db.sortedSetRemoveBulk([[`uid:${uid}:tids_read`, tid], [`tid:${tid}:bookmarks`, uid]]), db.sortedSetAdd(`uid:${uid}:tids_unread`, Date.now(), tid)]);
  };
