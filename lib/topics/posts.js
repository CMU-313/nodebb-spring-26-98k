...
module.exports = function (Topics) {
  ...
  Topics.addPostData = async function (postData, uid) {
    ...
    async function getPostUserData(field, method) {
      const uids = _.uniq(postData.filter(p => p && (activitypub.helpers.isUri(p[field]) || parseInt(p[field], 10) >= 0)).map(p => p[field]));
      const userData = await method(uids);
      return _.zipObject(uids, userData);
    }
    const [bookmarks, voteData, userData, editors, replies] = await Promise.all([posts.hasBookmarked(pids, uid), posts.getVoteStatusByPostIDs(pids, uid), getPostUserData('uid', async uids => await posts.getUserInfoForPosts(uids, uid)), getPostUserData('editor', async uids => await user.getUsersFields(uids, ['uid', 'username', 'userslug'])), getPostReplies(postData, uid), Topics.addParentPosts(postData, uid)]);
  ...
  Topics.modifyPostsByPrivilege = function (topicData, topicPrivileges) {
    ...
    topicData.posts.forEach(post => {
      if (post) {
        ...
        post.display_post_menu = topicPrivileges.isAdminOrMod || post.selfPost && !topicData.locked && !post.deleted || post.selfPost && post.deleted && parseInt(post.deleterUid, 10) === parseInt(topicPrivileges.uid, 10) || (loggedIn || topicData.postSharing.length) && !post.deleted;
  ...
  Topics.addParentPosts = async function (postData, callerUid) {
    let parentPids = postData.filter(p => p && p.hasOwnProperty('toPid') && (activitypub.helpers.isUri(p.toPid) || utils.isNumber(p.toPid))).map(postObj => postObj.toPid);
  ...
  async function getPostReplies(postData, callerUid) {
    ...
    const [arrayOfReplyPids, userSettings] = await Promise.all([db.getSortedSetsMembers(keys), user.getSettings(callerUid)]);
    ...
    const returnData = await Promise.all(arrayOfReplyPids.map(async (replyPids, idx) => {
      ...
      if (replyPids.length === 1) {
        ...
        currentData.hasSingleImmediateReply = currentPost && currentPost.tid === replyPost.tid && Math.abs(currentIndex - replyPost.index) === 1;
      }
      return currentData;
    }));
    return returnData;
