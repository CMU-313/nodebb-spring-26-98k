...
async function postTids(tids) {
  ...
  const uids = _.uniq(topicsData.map(topicData => topicData.uid)).filter(uid => uid);
  await Promise.all([].concat(topicsData.map(topicData => topics.restore(topicData.tid)), topicsData.map(topicData => topics.updateLastPostTimeFromLastPid(topicData.tid))));
  await Promise.all([].concat(sendNotifications(uids, topicsData), updateUserLastposttimes(uids, topicsData), updateGroupPosts(topicsData), federatePosts(uids, topicsData), ...topicsData.map(topicData => unpin(topicData.tid, topicData))));
}
Scheduled.pin = async function (tid, topicData) {
  return Promise.all([topics.setTopicField(tid, 'pinned', 1), db.sortedSetAdd(`cid:${topicData.cid}:tids:pinned`, Date.now(), tid), db.sortedSetsRemove([`cid:${topicData.cid}:tids`, `cid:${topicData.cid}:tids:create`, `cid:${topicData.cid}:tids:posts`, `cid:${topicData.cid}:tids:votes`, `cid:${topicData.cid}:tids:views`], tid)]);
};
...
async function sendNotifications(uids, topicsData) {
  ...
  await Promise.all(topicsData.map((t, idx) => user.notifications.sendTopicNotificationToFollowers(t.uid, t, postsData[idx])).concat(postsData.map(p => topics.notifyTagFollowers(p, p.uid))).concat(postsData.map(p => categories.notifyCategoryFollowers(p, p.uid))).concat(topicsData.map((t, idx) => socketHelpers.notifyNew(t.uid, 'newTopic', {
    posts: [postsData[idx]],
    topic: t
  }))));
...
async function shiftPostTimes(tid, timestamp) {
  ...
  return db.setObjectBulk(pids.map((pid, idx) => [`post:${pid}`, {
