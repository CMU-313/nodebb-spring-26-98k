...
module.exports = function (Topics) {
  ...
  async function toggleEndorse(tid, uid, endorse) {
    ...
    const now = Date.now();
    const promises = [Topics.setTopicField(tid, 'endorsed', endorse ? 1 : 0), Topics.events.log(tid, {
      type: endorse ? 'endorse' : 'unendorse',
      uid
    })];
    if (endorse) {
      promises.push(db.sortedSetAdd(`cid:${cid}:tids:endorsed`, now, tid));
    } else {
      promises.push(Topics.deleteTopicField(tid, 'endorsed'), db.sortedSetRemove(`cid:${cid}:tids:endorsed`, tid));
    }
    const results = await Promise.all(promises);
    topicData.isEndorsed = endorse;
    topicData.endorsed = endorse;
    topicData.events = results[1];
    plugins.hooks.fire('action:topic.endorse', {
      topic: _.clone(topicData),
      uid
    });
    return topicData;
  ...
  topicTools.move = async function (tid, data) {
    ...
    await db.sortedSetsRemove([`cid:${topicData.cid}:tids`, `cid:${topicData.cid}:tids:create`, `cid:${topicData.cid}:tids:pinned`, `cid:${topicData.cid}:tids:posts`, `cid:${topicData.cid}:tids:votes`, `cid:${topicData.cid}:tids:views`, `cid:${topicData.cid}:tids:lastposttime`, `cid:${topicData.cid}:recent_tids`, `cid:${topicData.cid}:uid:${topicData.uid}:tids`, ...tags.map(tag => `cid:${topicData.cid}:tag:${tag}:topics`)], tid);
    ...
    await Promise.all([Topics.setTopicFields(tid, {
      cid: cid,
      oldCid: oldCid
  ...
  topicTools.share = async function (tid, uid, timestamp = Date.now()) {
    ...
    await Promise.all([Topics.events.log(tid, {
      type: 'share',
      uid: uid
