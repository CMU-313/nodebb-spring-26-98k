...
module.exports = function (Topics) {
  Topics.createTopicFromPosts = async function (uid, title, pids, fromTid, cid) {
    ...
    const [mainPost, isAdminOrMod] = await Promise.all([posts.getPostData(mainPid), privileges.categories.isAdminOrMod(cid, uid)]);
    ...
    await Promise.all([Topics.setTopicFields(tid, {
      ...
      forkerUid: uid,
      forkTimestamp: now
    }), db.sortedSetsAdd(['topics:votes', `cid:${cid}:tids:votes`], mainPost.votes, tid), Topics.events.log(fromTid, {
      type: 'fork',
      uid,
      href: `/topic/${tid}`
    })]);
    if (utils.isNumber(pids[0])) {
      const {
        activity
      } = await activitypub.mocks.activities.create(pids[0], uid);
