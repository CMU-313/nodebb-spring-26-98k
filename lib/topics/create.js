...
module.exports = function (Topics) {
  Topics.create = async function (data) {
    ...
    const timestampedSortedSetKeys = ['topics:tid', `cid:${topicData.cid}:tids`, `cid:${topicData.cid}:tids:create`, `cid:${topicData.cid}:uid:${topicData.uid}:tids`];
    const countedSortedSetKeys = [...['views', 'posts', 'votes'].map(prop => `${utils.isNumber(tid) ? 'topics' : 'topicsRemote'}:${prop}`), `cid:${topicData.cid}:tids:votes`, `cid:${topicData.cid}:tids:posts`, `cid:${topicData.cid}:tids:views`];
    const scheduled = timestamp > Date.now();
    if (scheduled) {
      timestampedSortedSetKeys.push('topics:scheduled');
    }
    await Promise.all([db.sortedSetsAdd(timestampedSortedSetKeys, timestamp, topicData.tid), db.sortedSetsAdd(countedSortedSetKeys, 0, topicData.tid), user.addTopicIdToUser(topicData.uid, topicData.tid, timestamp), db.incrObjectField(`${utils.isNumber(topicData.cid) ? 'category' : 'categoryRemote'}:${topicData.cid}`, 'topic_count'), utils.isNumber(tid) ? db.incrObjectField('global', 'topicCount') : null, Topics.createTags(data.tags, topicData.tid, timestamp), scheduled ? Promise.resolve() : categories.updateRecentTid(topicData.cid, topicData.tid)]);
    if (scheduled) {
  ...
  Topics.post = async function (data) {
    ...
    const [categoryExists, canCreate, canTag, isAdmin] = await Promise.all([parseInt(data.cid, 10) > 0 ? categories.exists(data.cid) : true, privileges.categories.can('topics:create', data.cid, remoteUid ? -2 : uid), privileges.categories.can('topics:tag', data.cid, remoteUid ? -2 : uid), privileges.users.isAdministrator(uid)]);
  ...
  Topics.reply = async function (data) {
    ...
    const [topicData, isAdmin] = await Promise.all([Topics.getTopicData(tid), privileges.users.isAdministrator(uid)]);
  ...
  async function onNewPost({
    ...
    const [[postData], [userInfo]] = await Promise.all([posts.getPostSummaryByPids([pid], uid, {
      extraFields: ['attachments']
    }), posts.getUserInfoForPosts([postOwner], uid)]);
    await Promise.all([Topics.addParentPosts([postData], uid), Topics.syncBacklinks(postData), Topics.markAsRead([tid], uid)]);
