...
module.exports = function (Topics) {
  ...
  Topics.filterTags = async function (tags, cid) {
    ...
    tags = _.uniq(result.tags).map(tag => utils.cleanUpTag(tag, meta.config.maximumTagLength)).filter(tag => tag && tag.length >= (meta.config.minimumTagLength || 3));
  ...
  Topics.validateTags = async function (tags, cid, uid, tid = null) {
    ...
    tags = _.uniq(tags);
    const [categoryData, isPrivileged, currentTags] = await Promise.all([categories.getCategoryFields(cid, ['minTags', 'maxTags']), user.isPrivileged(uid), tid ? Topics.getTopicTags(tid) : []]);
  ...
  async function renameTag(tag, newTagName) {
    ...
    await batch.processSortedSet(`tag:${tag}:topics`, async tids => {
      ...
      await db.setObjectBulk(topicData.map(t => [`topic:${t.tid}`, {
        tags: t.tags.join(',')
      }]));
    }, {});
    ...
    if (followers.length) {
      ...
      await db.sortedSetAdd(`tag:${newTagName}:followers`, followers.map(item => item.score), followers.map(item => item.value));
    }
  ...
  async function removeTagsFromTopics(tags) {
    await async.eachLimit(tags, 50, async tag => {
      ...
      topicsTags = topicsTags.map(topicTags => topicTags.filter(topicTag => topicTag && topicTag !== tag));
      await db.setObjectBulk(tids.map((tid, index) => [`topic:${tid}`, {
  ...
  Topics.getCategoryTagsData = async function (cids, start, stop) {
    return await getFromSet(Array.isArray(cids) ? cids.map(cid => `cid:${cid}:tags`) : `cid:${cids}:tags`, start, stop);
  };
  ...
  Topics.getTopicsTagsObjects = async function (tids) {
    ...
    const tags = uniqueTopicTags.map(tag => ({
      value: tag
    }));
  ...
  Topics.updateTopicTags = async function (tid, tags) {
    ...
    plugins.hooks.fire('action:topic.updateTags', {
      tags,
      tid
    });
  };
  ...
  Topics.searchAndLoadTags = async function (data) {
    ...
    const tagData = Topics.getTagData(tags.map(tag => ({
      value: tag.value
    })));
  ...
  Topics.followTag = async (tag, uid) => {
    ...
    const now = Date.now();
    await db.sortedSetAddBulk([[`tag:${tag}:followers`, now, uid], [`uid:${uid}:followed_tags`, now, tag]]);
    plugins.hooks.fire('action:tags.follow', {
      tag,
      uid
    });
  };
  Topics.unfollowTag = async (tag, uid) => {
    ...
    await db.sortedSetRemoveBulk([[`tag:${tag}:followers`, uid], [`uid:${uid}:followed_tags`, tag]]);
    plugins.hooks.fire('action:tags.unfollow', {
      tag,
      uid
    });
  };
  Topics.notifyTagFollowers = async function (postData, exceptUid) {
    ...
    const [followersOfPoster, allFollowers, title] = await Promise.all([db.getSortedSetRange(`followers:${exceptUid}`, 0, -1), db.getSortedSetRange(tags.map(tag => `tag:${tag}:followers`), 0, -1), Topics.getTopicField(postData.topic.tid, 'title')]);
