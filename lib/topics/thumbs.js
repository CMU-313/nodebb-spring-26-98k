...
Thumbs.load = async function (topicData) {
  ...
  let hasThumbs = topicData.map((t, idx) => t && (parseInt(t.numThumbs, 10) > 0 || !!(hashes[idx] && hashes[idx].length) || hasUploads[idx]));
  ({
    hasThumbs
  } = await plugins.hooks.fire('filter:topics.hasThumbs', {
    topicData,
    hasThumbs
  }));
...
async function loadFromTopicData(topicData, options = {}) {
  ...
  if (!options.thumbsOnly) {
    ...
    const [mainPidUploads, mainPidAttachments] = await Promise.all([posts.uploads.list(mainPids), posts.attachments.get(mainPids)]);
    ...
    mainPidAttachments.forEach((attachments, idx) => {
      attachments = attachments.filter(attachment => !thumbs[idx].includes(attachment.url) && attachment.mediaType && attachment.mediaType.startsWith('image/'));
  ...
  ({
    thumbs: response
  } = await plugins.hooks.fire('filter:topics.getThumbs', {
    tids,
...
Thumbs.associate = async function ({
  ...
  if (Array.isArray(topicData.thumbs)) {
    ...
    const insertIndex = typeof score === 'number' && score >= 0 && score < topicData.thumbs.length ? score : topicData.thumbs.length;
    if (currentIdx !== -1) {
...
Thumbs.delete = async function (tid, relativePaths) {
  ...
  const toRemove = relativePaths.map(relativePath => topicData.thumbs.includes(relativePath) ? relativePath : null).filter(Boolean);
  if (toRemove.length) {
    const {
      mainPid
    } = topicData.mainPid;
