...
module.exports = function (Topics) {
  ...
  async function getTids(params) {
    ...
    if (params.term !== 'alltime') {
      if (params.sort === 'posts') {
        ...
        tids = await Topics.getLatestTidsFromSet(cids.map(cid => `cid:${cid}:tids:create`), 0, -1, params.term);
      }
  ...
  async function getTidsWithMostPostsInTerm(cids, uid, term) {
    ...
    const pids = await db.getSortedSetRevRangeByScore(cids.map(cid => `cid:${cid}:pids`), 0, 1000, '+inf', Date.now() - Topics.getSinceFromTerm(term));
  ...
  async function sortTids(tids, params) {
    ...
    const {
      sortMap,
      fields
    } = await plugins.hooks.fire('filter:topics.sortOptions', {
      params,
    ...
    const sortFn = sortMap.hasOwnProperty(params.sort) && sortMap[params.sort] ? sortMap[params.sort] : sortRecent;
  ...
  async function filterTids(tids, params) {
    ...
    tids = topicData.filter(t => t && t.cid && !isCidIgnored[t.cid] && (cids || parseInt(t.cid, 10) !== -1) && (!cids || cids.includes(String(t.cid))) && (!tags.length || tags.every(tag => t.tags.find(topicTag => topicTag.value === tag)))).map(t => t.tid);
