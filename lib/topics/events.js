...
Events.init = async () => {
  const {
    types
  } = await plugins.hooks.fire('filter:topicEvents.init', {
    types: Events._types
  });
  Events._types = types;
};
...
Events.translateSimple = translateSimple;
Events.translateEventArgs = translateEventArgs;
...
async function modifyEvent({
  ...
  const [users, fromCategories, userSettings] = await Promise.all([getUserInfo(events.map(event => event.uid).filter(Boolean)), getCategoryInfo(events.map(event => event.fromCid).filter(Boolean)), user.getSettings(uid)]);
...
Events.log = async (tid, payload) => {
  ...
  ({
    events
  } = await plugins.hooks.fire('filter:topic.events.log', {
    events
  }));
  return events;
};
