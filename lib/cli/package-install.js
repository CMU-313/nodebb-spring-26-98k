...
function sortDependencies(dependencies) {
  return Object.entries(dependencies).sort((a, b) => a < b ? -1 : 1).reduce((memo, pkg) => {
    memo[pkg[0]] = pkg[1];
    return memo;
  }, {});
}
...
pkgInstall.installAll = () => {
  ...
  const supportedPackageManagerList = exports.supportedPackageManager;
...
pkgInstall.preserveExtraneousPlugins = () => {
  ...
  const extraneous = packages.filter(pkgName => {
