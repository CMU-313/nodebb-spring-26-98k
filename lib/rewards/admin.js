...
rewards.delete = async function (data) {
  await Promise.all([db.sortedSetRemove('rewards:list', data.id), db.delete(`rewards:id:${data.id}`), db.delete(`rewards:id:${data.id}:rewards`)]);
};
...
async function getActiveRewards() {
  ...
  const rewardData = await Promise.all(rewardsList.map(async id => {
    const [main, rewards] = await Promise.all([db.getObject(`rewards:id:${id}`), db.getObject(`rewards:id:${id}:rewards`)]);
    if (main) {
