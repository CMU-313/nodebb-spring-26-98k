...
mongoModule.info = async function (db) {
  ...
  let [serverStatus, stats, listCollections] = await Promise.all([getServerStatus(), db.command({
    dbStats: 1
...
async function getCollectionStats(db) {
  ...
  const cols = await Promise.all(items.map(collection => db.collection(collection.name).aggregate([{
    $collStats: {
      latencyStats: {},
      storageStats: {},
      count: {}
