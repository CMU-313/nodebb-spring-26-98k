...
async function searchInContent(data) {
  ...
  if (inTopic) {
    ...
    if (result) {
      ...
      [pids, tids] = await Promise.all([doSearch('post', ['posts', 'titlesposts']), doSearch('topic', ['titles', 'titlesposts'])]);
    }
  }
    ...
    if (!exists) {
      ...
      if (activitypub._constants.acceptedPostTypes.includes(type)) {
        ...
        if (!exists) {
          ({
            tid
          } = await activitypub.notes.assert(uid, id));
        } else {
...
async function filterAndSort(pids, data) {
  if (data.sortBy === 'relevance' && !data.replies && !data.timeRange && !data.hasTags && data.searchIn !== 'bookmarks' && !plugins.hooks.hasListeners('filter:search.filterAndSort')) {
    return pids;
  }
