...
privsPosts.filter = async function (privilege, pids, uid) {
  ...
  pids = postData.filter(post => post.topic && cidsSet.has(post.topic.cid) && (privsTopics.canViewDeletedScheduled({
...
privsPosts.canEdit = async function (pid, uid) {
  ...
  if (!isRemote && !results.isMod && meta.config.postEditDuration && Date.now() - results.postData.timestamp > meta.config.postEditDuration * 1000) {
    return {
      flag: false,
      message: `[[error:post-edit-duration-expired, ${meta.config.postEditDuration}]]`
    };
  }
  if (!isRemote && !results.isMod && meta.config.newbiePostEditDuration > 0 && meta.config.newbieReputationThreshold > results.userData.reputation && Date.now() - results.postData.timestamp > meta.config.newbiePostEditDuration * 1000) {
    return {
      flag: false,
