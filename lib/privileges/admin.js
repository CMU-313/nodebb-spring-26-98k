...
const _privilegeMap = new Map([['admin:dashboard', {
  ...
  label: '[[admin/manage/privileges:admin-users]]',
  type: 'admin'
}], ['admin:groups', {
  label: '[[admin/manage/privileges:admin-groups]]',
  type: 'admin'
}], ['admin:tags', {
  label: '[[admin/manage/privileges:admin-tags]]',
  type: 'admin'
}], ['admin:settings', {
  label: '[[admin/manage/privileges:admin-settings]]',
  type: 'admin'
}]]);
...
privsAdmin.resolve = path => {
  ...
  const found = Object.entries(privsAdmin.routePrefixMap).filter(entry => path.startsWith(entry[0])).sort((entry1, entry2) => entry2[0].length - entry1[0].length);
  if (!found.length) {
    return undefined;
  }
  return found[0][1];
