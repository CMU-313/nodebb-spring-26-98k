...
Groups.get = async function (groupName, options) {
  ...
  const [groupData, members, isMember, isPending, isInvited, isOwner, isAdmin, isGlobalMod] = await Promise.all([Groups.getGroupData(groupName), Groups.getOwnersAndMembers(groupName, options.uid, 0, stop), Groups.isMember(options.uid, groupName), Groups.isPending(options.uid, groupName), Groups.isInvited(options.uid, groupName), Groups.ownership.isOwner(options.uid, groupName), privileges.admin.can('admin:groups', options.uid), user.isGlobalModerator(options.uid)]);
  if (!groupData) {
    return null;
  }
  groupData.isOwner = isOwner || isAdmin || isGlobalMod && !groupData.system;
  if (groupData.isOwner) {
    [groupData.pending, groupData.invited] = await Promise.all([Groups.getPending(groupName), Groups.getInvites(groupName)]);
  }
