...
module.exports = function (Groups) {
  Groups.create = async function (data) {
    ...
    const [exists, privGroupExists] = await Promise.all([meta.slugTaken(data.name), privilegeGroupExists(data.name)]);
    ...
    if (!isHidden && !isSystem) {
      await db.sortedSetAddBulk([['groups:visible:createtime', timestamp, groupData.name], ['groups:visible:memberCount', groupData.memberCount, groupData.name], ['groups:visible:name', 0, `${groupData.name.toLowerCase()}:${groupData.name}`]]);
    }
    ...
    plugins.hooks.fire('action:group.create', {
      group: groupData
    });
    return groupData;
  };
