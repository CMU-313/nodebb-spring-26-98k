...
module.exports = function (Groups) {
  Groups.update = async function (groupName, values) {
    ...
    ({
      values
    } = await plugins.hooks.fire('filter:group.update', {
      groupName: groupName,
      values: values
    }));
  ...
  async function updateVisibility(groupName, hidden) {
    if (hidden) {
      await db.sortedSetRemoveBulk([['groups:visible:createtime', groupName], ['groups:visible:memberCount', groupName], ['groups:visible:name', `${groupName.toLowerCase()}:${groupName}`]]);
      return;
    }
    const groupData = await db.getObjectFields(`group:${groupName}`, ['createtime', 'memberCount']);
    await db.sortedSetAddBulk([['groups:visible:createtime', groupData.createtime, groupName], ['groups:visible:memberCount', groupData.memberCount, groupName], ['groups:visible:name', 0, `${groupName.toLowerCase()}:${groupName}`]]);
  }
  ...
  async function showHide(groupName, hidden) {
    ...
    await Promise.all([db.setObjectField(`group:${groupName}`, 'hidden', hidden ? 1 : 0), updateVisibility(groupName, hidden)]);
  }
  ...
  async function updateConfig(oldName, newName) {
    const configKeys = ['groupsExemptFromPostQueue', 'groupsExemptFromNewUserRestrictions', 'groupsExemptFromMaintenanceMode'];
  ...
  async function updateChatRooms(oldName, newName) {
    ...
    roomData.forEach(room => {
      if (room && room.public && Array.isArray(room.groups) && room.groups.includes(oldName)) {
        ...
        bulkSet.push([`chat:room:${room.roomId}`, {
