...
module.exports = function (Groups) {
  Groups.leave = async function (groupNames, uid) {
    ...
    await Promise.all([db.sortedSetRemove(groupsToLeave.map(groupName => `group:${groupName}:members`), uid), db.setRemove(groupsToLeave.map(groupName => `group:${groupName}:owners`), uid), db.decrObjectField(groupsToLeave.map(groupName => `group:${groupName}`), 'memberCount')]);
    ...
    await Promise.all([clearGroupTitleIfSet(groupsToLeave, uid), leavePublicRooms(groupsToLeave, uid)]);
  ...
  async function leavePublicRooms(groupNames, uid) {
    ...
    const roomData = allRoomData.filter(room => room && room.groups.some(group => groupNames.includes(group)));
    const isMemberOfAny = _.zipObject(roomData.map(r => r.roomId), await Promise.all(roomData.map(r => Groups.isMemberOfAny(uid, r.groups))));
