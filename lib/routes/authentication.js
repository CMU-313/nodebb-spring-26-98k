...
Auth.reloadRoutes = async function (params) {
  ...
  loginStrategies.forEach(strategy => {
    ...
    router[strategy.callbackMethod || 'get'](strategy.callbackURL, (req, res, next) => {
      ...
      }), async.apply(controllers.authentication.onSuccessfulLogin, req, res.locals.user.uid)], err => {
        if (err) {
          return next(err);
        }
  ...
  const middlewares = [upload.any(), Auth.middleware.applyCSRF, Auth.middleware.applyBlacklist];
