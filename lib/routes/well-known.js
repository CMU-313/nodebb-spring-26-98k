...
module.exports = function (app, middleware, controllers) {
  ...
  app.get('/nodeinfo/2.0(.json)?', helpers.tryRoute(async (req, res) => {
    ...
    const [{
      ...
      topicCount,
      userCount
    }, activeMonth, activeHalfyear] = await Promise.all([db.getObjectFields('global', ['postCount', 'topicCount', 'userCount']), db.sortedSetCount('users:online', oneMonthAgo.getTime(), '+inf'), db.sortedSetCount('users:online', sixMonthsAgo.getTime(), '+inf')]);
    res.json({
      version: '2.0',
      software: {
        name: 'NodeBB',
