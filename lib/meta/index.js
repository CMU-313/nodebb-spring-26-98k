...
Meta.slugTaken = async function (slug) {
  ...
  const [userExists, groupExists, categoryExists] = await Promise.all([user.existsBySlug(slug), groups.existsBySlug(slug), categories.existsByHandle(slug)]);
  return isArray ? slug.map((s, i) => userExists[i] || groupExists[i] || categoryExists[i]) : userExists || groupExists || categoryExists;
};
...
Meta.getSessionTTLSeconds = function () {
  ...
  const ttl = ttlSeconds || ttlDays || 1209600;
  return ttl;
};
