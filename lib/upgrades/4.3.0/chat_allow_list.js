...
module.exports = {
  ...
  method: async function () {
    ...
    await batch.processSortedSet('users:joindate', async uids => {
      ...
      const [userSettings, followingUids] = await Promise.all([db.getObjects(keys), db.getSortedSetsMembers(uids.map(uid => `following:${uid}`))]);
      const bulkSet = [];
      userSettings.forEach((settings, idx) => {
        if (settings) {
          ...
          if (parseInt(settings.restrictChat, 10) === 1 && followingUidsOfThisUser.length > 0) {
            bulkSet.push([`user:${uid}:settings`, {
