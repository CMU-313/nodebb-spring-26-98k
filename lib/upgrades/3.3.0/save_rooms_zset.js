...
module.exports = {
  ...
  method: async function () {
    ...
    await batch.processArray(allRoomIds, async roomIds => {
      ...
      const arrayOfUids = await Promise.all(roomIds.map(roomId => db.getSortedSetRangeWithScores(`chat:room:${roomId}:uids`, 0, 0)));
      const timestamps = roomIds.map((id, idx) => arrayOfUids[idx].length ? arrayOfUids[idx][0].score || timestamp : timestamp);
