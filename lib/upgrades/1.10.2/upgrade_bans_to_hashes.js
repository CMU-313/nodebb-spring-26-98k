...
module.exports = {
  ...
  method: async function () {
    ...
    await batch.processSortedSet('users:joindate', async uids => {
      ...
      const [allUserData, allBans] = await Promise.all([db.getObjectsFields(uids.map(uid => `user:${uid}`), ['banned', 'banned:expire', 'joindate', 'lastposttime', 'lastonline']), db.getSortedSetsMembersWithScores(uids.map(uid => `uid:${uid}:bans`))]);
      await Promise.all(uids.map(async (uid, index) => {
        ...
        if (!bans.length && parseInt(userData.banned, 10)) {
          const banTimestamp = userData.lastonline || userData.lastposttime || userData.joindate || Date.now();
...
async function addBan(uid, key, data) {
  await Promise.all([db.setObject(key, data), db.sortedSetAdd(`uid:${uid}:bans:timestamp`, data.timestamp, key)]);
}