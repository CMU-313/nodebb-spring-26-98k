...
module.exports = {
  ...
  method: async function () {
    ...
    await batch.processSortedSet('users:joindate', async uids => {
      const userSettings = await db.getObjectsFields(uids.map(uid => `user:${uid}:settings`), ['sendChatNotifications', 'sendPostNotifications']);
      const bulkSet = [];
      ...
      await db.deleteObjectFields(uids.map(uid => `user:${uid}:settings`), ['sendChatNotifications', 'sendPostNotifications']);
