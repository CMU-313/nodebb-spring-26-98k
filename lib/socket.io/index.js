...
function onDisconnect(socket) {
  ...
  plugins.hooks.fire('action:sockets.disconnect', {
    socket: socket
  });
}
async function onConnect(socket) {
  ...
  plugins.hooks.fire('action:sockets.connect', {
    socket: socket
  });
}
...
async function onMessage(socket, payload) {
  ...
  try {
    ...
    if (typeof event !== 'string') {
      ...
      return callback({
        message: `[[error:invalid-event, ${escapedName}]]`
      });
    }
...
async function validateSession(socket, errorMsg) {
  ...
  const {
    sessionId
  } = await plugins.hooks.fire('filter:sockets.sessionId', {
