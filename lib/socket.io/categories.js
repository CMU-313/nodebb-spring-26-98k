...
SocketCategories.getRecentReplies = async function (socket, cid) {
  ...
  return await api.categories.getPosts(socket, {
    cid
  });
};
SocketCategories.get = async function (socket) {
  ...
  const {
    categories
  } = await api.categories.list(socket);
  return categories;
};
...
SocketCategories.getTopicCount = async function (socket, cid) {
  ...
  const {
    count
  } = await api.categories.getTopicCount(socket, {
    cid
  });
  return count;
};
...
SocketCategories.loadMoreSubCategories = async function (socket, data) {
  ...
  const {
    categories: children
  } = await api.categories.getChildren(socket, data);
  return children;
};
