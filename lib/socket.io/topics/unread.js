...
module.exports = function (SocketTopics) {
  SocketTopics.markAsRead = async function (socket, tids) {
    ...
    await Promise.all(tids.map(async tid => api.topics.markRead(socket, {
      tid
    })));
  };
  ...
  SocketTopics.markUnread = async function (socket, tid) {
    ...
    await api.topics.markUnread(socket, {
      tid
    });
  };
  SocketTopics.markAsUnreadForAll = async function (socket, tids) {
    ...
    await Promise.all(tids.map(async tid => api.topics.bump(socket, {
      tid
    })));
  };
};