...
module.exports = function (SocketPosts) {
  SocketPosts.loadPostTools = async function (socket, data) {
    ...
    const {
      tools
    } = await plugins.hooks.fire('filter:post.tools', {
      pid: data.pid,
      post: postData,
      uid: socket.uid,
      tools: []
  ...
  SocketPosts.changeOwner = async function (socket, data) {
    ...
    await api.posts.changeOwner(socket, {
      pids: data.pids,
      uid: data.toUid
    });
  };
  ...
  async function checkEditorPrivilege(uid, pid) {
    ...
    const [isAdminOrMod, owner] = await Promise.all([privileges.categories.isAdminOrMod(cid, uid), posts.getPostField(pid, 'uid')]);
