...
SocketModules.chats.getRaw = async function (socket, data) {
  ...
  const {
    content
  } = await api.chats.getRawMessage(socket, {
    mid: data.mid,
    roomId
  });
  return content;
...
SocketModules.chats.isDnD = async function (socket, uid) {
  ...
  const {
    status
  } = await api.users.getStatus(socket, {
    uid
  });
  return status === 'dnd';
};
...
SocketModules.chats.hasPrivateChat = async function (socket, uid) {
  ...
  const {
    roomId
  } = await api.users.getPrivateRoomId(socket, {
    uid
  });
  return roomId;
};
SocketModules.chats.getIP = async function (socket, mid) {
  ...
  const {
    ip
  } = await api.chats.getIpAddress(socket, {
    mid
  });
  return ip;
};
SocketModules.chats.getUnreadCount = async function (socket) {
  ...
  const {
    count
  } = await api.chats.getUnread(socket);
  return count;
};
...
async function joinLeave(socket, roomIds, method, prefix = 'chat_room') {
  ...
  if (roomIds.length) {
    const [isAdmin, inRooms, roomData] = await Promise.all([user.isAdministrator(socket.uid), Messaging.isUserInRoom(socket.uid, roomIds), Messaging.getRoomsData(roomIds, ['public', 'groups'])]);
...
SocketModules.chats.loadPinnedMessages = async (socket, data) => {
  ...
  const {
    messages
  } = await api.chats.getPinnedMessages(socket, data);
  return messages;
};
