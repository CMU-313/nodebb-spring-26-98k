...
SocketUser.reset.commit = async function (socket, data) {
  ...
  const [uid] = await Promise.all([db.getObjectField('reset:uid', data.code), user.reset.commit(data.code, data.password), plugins.hooks.fire('action:password.reset', {
    uid: socket.uid
  })]);
  await events.log({
    type: 'password-reset',
...
SocketUser.getUnreadCounts = async function (socket) {
  ...
  const results = await utils.promiseParallel({
    unreadCounts: topics.getUnreadTids({
      uid: socket.uid,
      count: true
    }),
...
SocketUser.setModerationNote = async function (socket, data) {
  ...
  await user.appendModerationNote({
    uid: data.uid,
    noteData
  });
