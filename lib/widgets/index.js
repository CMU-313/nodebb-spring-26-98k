...
async function renderLocation(location, data, uid, options, config) {
  ...
  const renderedWidgets = await Promise.all(widgetsAtLocation.map(widget => renderWidget(widget, uid, options, config, location)));
...
async function renderWidget(widget, uid, options, config, location) {
  ...
  const templateData = _.assign({}, options.templateData, {
    config: config
  });
  try {
...
widgets.checkVisibility = async function (data, uid) {
  ...
  const isExpired = data.startDate && Date.now() < new Date(data.startDate).getTime() || data.endDate && Date.now() > new Date(data.endDate).getTime();
...
widgets.moveMissingAreasToDrafts = async function () {
  ...
  try {
    ...
    const [available, draftWidgets] = await Promise.all([widgets.getAvailableAreas(), widgets.getArea('global', 'drafts')]);
    ...
    for (const [template, tplLocations] of Object.entries(locations)) {
      for (const location of tplLocations) {
        const locationExists = available.find(area => area.template === template && area.location === location);
        if (!locationExists) {
...
widgets.reset = async function () {
  const [areas, drafts] = await Promise.all([widgets.getAvailableAreas(), widgets.getArea('global', 'drafts')]);
