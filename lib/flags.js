...
Flags.getCount = async function ({
  ...
  filters = filters || {};
  const flagIds = await Flags.getFlagIdsWithFilters({
    filters,
    uid,
    query
  });
...
Flags.create = async function (type, id, uid, reason, timestamp, forceFlag = false, notifyRemote = false) {
  ...
  const [flagExists, targetExists,, targetFlagged, targetUid, targetCid] = await Promise.all([Flags.exists(type, id, uid), Flags.targetExists(type, id), Flags.canFlag(type, id, uid, forceFlag), Flags.targetFlagged(type, id), Flags.getTargetUid(type, id), Flags.getTargetCid(type, id)]);
  ...
  if (targetFlagged) {
    ...
    await Promise.all([Flags.addReport(flagId, type, id, uid, reason, timestamp, targetUid, notifyRemote), Flags.update(flagId, uid, {
      state: 'open',
      report: 'added'
    })]);
  ...
  batched.push(db.setObject(`flag:${flagId}`, {
    ...
    targetUid: targetUid,
    datetime: timestamp
  }), Flags.addReport(flagId, type, id, uid, reason, timestamp, targetUid, notifyRemote), db.sortedSetAdd('flags:datetime', timestamp, flagId), db.sortedSetAdd(`flags:byType:${type}`, timestamp, flagId), db.sortedSetIncrBy('flags:byTarget', 1, [type, id].join(':')), analytics.increment('flags'));
  if (targetUid) {
    batched.push(db.sortedSetAdd(`flags:byTargetUid:${targetUid}`, timestamp, flagId));
  }
  if (targetCid) {
    batched.push(db.sortedSetAdd(`flags:byCid:${targetCid}`, timestamp, flagId));
  }
  if (type === 'post') {
    batched.push(db.sortedSetAdd(`flags:byPid:${id}`, timestamp, flagId), posts.setPostField(id, 'flagId', flagId));
...
Flags.purge = async function (flagIds) {
  ...
  await Promise.all([db.sortedSetRemoveBulk([...flagData.map(flagObj => [`flags:byType:${flagObj.type}`, flagObj.flagId]), ...flagData.map(flagObj => [`flags:byState:${flagObj.state}`, flagObj.flagId]), ...removeReporters, ...postFlags.map(flagObj => [`flags:byPid:${flagObj.targetId}`, flagObj.flagId]), ...assignedFlags.map(flagObj => [`flags:byAssignee:${flagObj.assignee}`, flagObj.flagId]), ...userFlags.map(flagObj => [`flags:byTargetUid:${flagObj.targetUid}`, flagObj.flagId])]), db.deleteObjectFields(postFlags.map(flagObj => `post:${flagObj.targetId}`, ['flagId'])), db.deleteObjectFields(userFlags.map(flagObj => `user:${flagObj.targetId}`, ['flagId'])), db.deleteAll([...flagIds.map(flagId => `flag:${flagId}`), ...flagIds.map(flagId => `flag:${flagId}:notes`), ...flagIds.map(flagId => `flag:${flagId}:reports`), ...flagIds.map(flagId => `flag:${flagId}:history`)]), db.sortedSetRemove(cids.map(cid => `flags:byCid:${cid}`), flagIds), db.sortedSetRemove('flags:datetime', flagIds), db.sortedSetRemove('flags:byTarget', flagData.map(flagObj => [flagObj.type, flagObj.targetId].join(':'))), flagData.flatMap(async (flagObj, i) => allReporterUids[i].map(async uid => {
...
Flags.addReport = async function (flagId, type, id, uid, reason, timestamp, targetUid, notifyRemote) {
  await db.sortedSetAddBulk([[`flags:byReporter:${uid}`, timestamp, flagId], [`flag:${flagId}:reports`, timestamp, [uid, reason].join(';')], ['flags:hash', flagId, [type, id, uid].join(':')]]);
  if (notifyRemote && activitypub.helpers.isUri(id)) {
...
Flags.rescindReport = async (type, id, uid) => {
  ...
  await db.sortedSetRemoveBulk([[`flags:byReporter:${uid}`, flagId], [`flag:${flagId}:reports`, [uid, reason].join(';')], ['flags:hash', [type, id, uid].join(':')]]);
...
Flags.canView = async (flagId, uid) => {
  ...
  const [{
    type,
    targetId
  }, isAdminOrGlobalMod] = await Promise.all([db.getObject(`flag:${flagId}`), user.isAdminOrGlobalMod(uid)]);
  if (type === 'post') {
...
Flags.canFlag = async function (type, id, uid, skipLimitCheck = false) {
  ...
  const now = Date.now();
  const [flagIds, canRead, isPrivileged] = await Promise.all([db.getSortedSetRangeByScore(`flags:byReporter:${uid}`, 0, -1, now - oneday, '+inf'), privileges.posts.can('topics:read', id, uid), user.isPrivileged(uid)]);
