...
middleware.assertS2S = async function (req, res, next) {
  ...
  const pass = activitypub.helpers.assertAccept(accept) || contentType && activitypub._constants.acceptableTypes.includes(contentType);
  if (!pass) {
    return next('route');
  }
  next();
