...
Assert.user = helpers.try(async (req, res, next) => {
  ...
  if (uid !== -2 && ((utils.isNumber(uid) || activitypub.helpers.isUri(uid)) && (await user.exists(uid)) || uid.indexOf('@') !== -1 && (await user.existsBySlug(uid)))) {
    return next();
  }
...
Assert.room = helpers.try(async (req, res, next) => {
  ...
  const [exists, inRoom] = await Promise.all([messaging.roomExists(req.params.roomId), messaging.isUserInRoom(req.uid, req.params.roomId)]);
  if (!exists) {
...
Assert.message = helpers.try(async (req, res, next) => {
  ...
  if (!(await messaging.messageExists(req.params.mid)) || !(await messaging.canViewMessage(req.params.mid, roomId || req.params.roomId, req.uid))) {
