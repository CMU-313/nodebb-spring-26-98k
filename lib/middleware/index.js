...
middleware.pageView = helpers.try(async (req, res, next) => {
  ...
  plugins.hooks.fire('action:middleware.pageView', {
    req: req
  });
});
...
middleware.addUploadHeaders = function addUploadHeaders(req, res, next) {
  ...
  const unsafeExtensions = ['.html', '.htm', '.xhtml', '.mht', '.mhtml', '.stm', '.shtm', '.shtml', '.svg', '.svgz', '.xml', '.xsl', '.xslt'];
...
middleware.checkRequired = function (fields, req, res, next) {
  const missing = fields.filter(field => req.body && !req.body.hasOwnProperty(field) && !req.query.hasOwnProperty(field));
  if (!missing.length) {
    return next();
  }
