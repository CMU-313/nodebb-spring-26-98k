...
module.exports = function (middleware) {
  ...
  middleware.authenticateRequest = helpers.try(async (req, res, next) => {
    const {
      skip
    } = await plugins.hooks.fire('filter:middleware.authenticate', {
      skip: {
  ...
  middleware.redirectUidToUserslug = helpers.try(async (req, res, next) => {
    ...
    const [canView, userslug] = await Promise.all([privileges.global.can('view:users', req.uid), user.getUserField(uid, 'userslug')]);
  ...
  middleware.registrationComplete = async function registrationComplete(req, res, next) {
    ...
    const {
      allowed
    } = await plugins.hooks.fire('filter:middleware.registrationComplete', {
  ...
  async function requiresEmailConfirmation(req) {
    ...
    const [confirmed, isAdmin] = await Promise.all([groups.isMember(req.uid, 'verified-users'), user.isAdministrator(req.uid)]);
    ...
    const [registeredAllowed, verifiedAllowed] = await Promise.all([privilegeHelpers.isAllowedTo([privilege], 'registered-users', cid), privilegeHelpers.isAllowedTo([privilege], 'verified-users', cid)]);
