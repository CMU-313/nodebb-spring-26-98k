...
module.exports = function (Messaging) {
  ...
  Messaging.getMessagesData = async (mids, uid, roomId, isNew) => {
    ...
    const users = await user.getUsersFields(messages.map(msg => msg && msg.fromuid), ['uid', 'username', 'userslug', 'picture', 'status', 'banned']);
    ...
    if (messages.length > 1) {
      ...
      if (index > 0) {
        ...
        if (messages[0].timestamp > fields.timestamp + Messaging.newMessageCutoff || messages[0].fromuid !== fields.fromuid || messages[0].system || messages[0].toMid) {
  ...
  async function addParentMessages(messages, uid, roomId) {
    ...
    const parentMessages = await Messaging.getMessagesFields(parentMids, ['mid', 'fromuid', 'content', 'timestamp', 'deleted']);
    const parentUids = _.uniq(parentMessages.map(msg => msg && msg.fromuid));
    const usersMap = _.zipObject(parentUids, await user.getUsersFields(parentUids, ['uid', 'username', 'userslug', 'picture']));
