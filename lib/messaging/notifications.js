...
module.exports = function (Messaging) {
  ...
  Messaging.getUidsNotificationSetting = async (uids, roomId) => {
    const [settings, roomData] = await Promise.all([db.getObjectFields(`chat:room:${roomId}:notification:settings`, uids), Messaging.getRoomData(roomId, ['notificationSetting'])]);
  ...
  Messaging.notifyUsersInRoom = async (fromUid, roomId, messageObj) => {
    ...
    if (!isPublic) {
      ...
      unreadData.teaser = {
        content: validator.escape(String(utils.stripHTMLTags(utils.decodeHTMLEntities(messageObj.content)))),
    ...
    try {
      await Promise.all([sendNotification(fromUid, roomId, messageObj), !isPublic && utils.isNumber(fromUid) ? activitypub.out.create.privateNote(messageObj) : null]);
    } catch (err) {
      winston.error(`[messaging/notifications] Unabled to send notification\n${err.stack}`);
    }
  };
  async function sendNotification(fromUid, roomId, messageObj) {
    const [settings, roomData, realtimeUids] = await Promise.all([db.getObject(`chat:room:${roomId}:notification:settings`), Messaging.getRoomData(roomId), io.getUidsInRoom(`chat_room_${roomId}`)]);
    ...
    await batch.processSortedSet(`chat:room:${roomId}:uids:online`, async uids => {
      uids = uids.filter(uid => utils.isNumber(uid) && parseInt(settings && settings[uid] || roomDefault, 10) === ALLMESSAGES && String(fromUid) !== String(uid) && !realtimeUids.includes(parseInt(uid, 10)));
    ...
    if (uidsToNotify.length) {
      ...
      const notifData = {
        ...
        subject: roomData.roomName ? `[[email:notif.chat.new-message-from-user-in-room, ${displayname}, ${roomName}]]` : `[[email:notif.chat.new-message-from-user, ${displayname}]]`,
