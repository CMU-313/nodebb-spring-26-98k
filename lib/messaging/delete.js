...
module.exports = function (Messaging) {
  ...
  async function doDeleteRestore(mid, state, uid) {
    ...
    const msgData = await Messaging.getMessageFields(mid, ['mid', 'fromuid', 'deleted', 'roomId', 'content', 'system']);
    ...
    if (state === 1 && ioRoom) {
      ...
      plugins.hooks.fire('action:messaging.delete', {
        message: msgData
      });
      ...
      plugins.hooks.fire('action:messaging.restore', {
        message: msgData
      });
    }
