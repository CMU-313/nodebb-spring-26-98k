...
const intFields = ['roomId', 'timestamp', 'userCount', 'messageCount', 'joinLeaveMessages'];
module.exports = function (Messaging) {
  ...
  Messaging.getRoomsData = async (roomIds, fields = []) => {
    ...
    const roomData = await db.getObjects(roomIds.map(roomId => `chat:room:${roomId}`), fields);
    ...
    rooms.forEach(data => {
      if (data) {
        ...
        if (!fields.length || fields.includes('notificationSetting')) {
          data.notificationSetting = data.notificationSetting || (data.public ? Messaging.notificationSettings.ATMENTION : Messaging.notificationSettings.ALLMESSAGES);
        }
  ...
  Messaging.newRoom = async (uid, data) => {
    ...
    await Promise.all([db.setObject(`chat:room:${roomId}`, room), db.sortedSetAdd('chat:rooms', now, roomId), db.sortedSetAdd(`chat:room:${roomId}:owners`, now, uid), db.sortedSetsAdd([`chat:room:${roomId}:uids`, `chat:room:${roomId}:uids:online`], now, uid)]);
    await Promise.all([Messaging.addUsersToRoom(uid, data.uids, roomId), isPublic ? db.sortedSetAddBulk([['chat:rooms:public', now, roomId], ['chat:rooms:public:order', roomId, roomId]]) : Messaging.addRoomToUsers(roomId, [uid].concat(data.uids), now)]);
  ...
  Messaging.isUserInRoom = async (uid, roomIds) => {
    ...
    const inRooms = await db.isMemberOfSortedSets(roomIds.map(id => `chat:room:${id}:uids`), uid);
  ...
  async function addUidsToRoom(uids, roomId) {
    ...
    await Promise.all([db.sortedSetAdd(`chat:room:${roomId}:uids`, timestamps, uids), db.sortedSetAdd(`chat:room:${roomId}:uids:online`, timestamps, uids)]);
  ...
  Messaging.removeUsersFromRoom = async (uid, uids, roomId) => {
    const [isOwner, userCount] = await Promise.all([Messaging.isRoomOwner(uid, roomId), Messaging.getUserCountInRoom(roomId)]);
  ...
  async function updateUserCount(roomIds) {
    ...
    await db.setObjectBulk([...groupChats.map(id => [`chat:room:${id}`, {
      groupChat: 1,
      userCount: countMap[id]
  ...
  Messaging.leaveRoom = async (uids, roomId) => {
    ...
    const keys = uids.map(uid => `uid:${uid}:chat:rooms`).concat(uids.map(uid => `uid:${uid}:chat:rooms:unread`));
    await Promise.all([db.sortedSetRemove([`chat:room:${roomId}:uids`, `chat:room:${roomId}:owners`, `chat:room:${roomId}:uids:online`], uids), db.sortedSetsRemove(keys, roomId)]);
    if (await joinLeaveMessagesEnabled(roomId)) {
  ...
  Messaging.leaveRooms = async (uid, roomIds) => {
    ...
    const roomKeys = [...roomIds.map(roomId => `chat:room:${roomId}:uids`), ...roomIds.map(roomId => `chat:room:${roomId}:owners`), ...roomIds.map(roomId => `chat:room:${roomId}:uids:online`)];
    await Promise.all([db.sortedSetsRemove(roomKeys, uid), db.sortedSetRemove([`uid:${uid}:chat:rooms`, `uid:${uid}:chat:rooms:unread`], roomIds)]);
    await Promise.all(roomIds.map(async roomId => {
      await updateOwner(roomId);
      if (await joinLeaveMessagesEnabled(roomId)) {
  ...
  Messaging.getUidsInRoomFromSet = async (set, start, stop, reverse = false) => db[reverse ? 'getSortedSetRevRange' : 'getSortedSetRange'](set, start, stop);
  Messaging.getUidsInRoom = async (roomId, start, stop, reverse = false) => db[reverse ? 'getSortedSetRevRange' : 'getSortedSetRange'](`chat:room:${roomId}:uids`, start, stop);
  Messaging.getUsersInRoom = async (roomId, start, stop, reverse = false) => {
    const users = await Messaging.getUsersInRoomFromSet(`chat:room:${roomId}:uids`, roomId, start, stop, reverse);
    return users;
  };
  Messaging.getUsersInRoomFromSet = async (set, roomId, start, stop, reverse = false) => {
    ...
    const [users, isOwners] = await Promise.all([user.getUsersFields(uids, ['uid', 'username', 'picture', 'status']), Messaging.isRoomOwner(uids, roomId)]);
  ...
  Messaging.loadRoom = async (uid, data) => {
    ...
    const [room, inRoom, canChat, isAdmin, isGlobalMod] = await Promise.all([Messaging.getRoomData(roomId), Messaging.isUserInRoom(uid, roomId), privileges.global.can(['chat', 'chat:privileged'], uid), user.isAdministrator(uid), user.isGlobalModerator(uid)]);
    if (!room || !room.public && !inRoom || room.public && Array.isArray(room.groups) && room.groups.length && !isAdmin && !(await groups.isMemberOfAny(uid, room.groups))) {
      return null;
    }
    ...
    async function getNotificationOptions() {
      ...
      return {
        options,
        selectedIcon: labels[currentSetting].icon
      };
    }
    const [canReply, users, messages, settings, isOwner, onlineUids, notifOptions] = await Promise.all([Messaging.canReply(roomId, uid), Messaging.getUsersInRoomFromSet(`chat:room:${roomId}:uids:online`, roomId, 0, 39, true), Messaging.getMessages({
      callerUid: uid,
      start: data.start || 0,
      uid: data.uid || uid,
      roomId: roomId,
      isNew: false
    }), user.getSettings(uid), Messaging.isRoomOwner(uid, roomId), io.getUidsInRoom(`chat_room_${roomId}`), getNotificationOptions(), Messaging.markRoomNotificationsRead(uid, roomId)]);
    users.forEach(user => {
      if (user) {
  ...
  const globalUserGroups = ['registered-users', 'verified-users', 'unverified-users', 'banned-users'];
