...
module.exports = function (Messaging) {
  ...
  Messaging.checkContent = async (content, local = true) => {
    ...
    ({
      content,
      length
    } = await plugins.hooks.fire('filter:messaging.checkContent', {
      content,
      length
    }));
    if (!content) {
  ...
  Messaging.addMessage = async data => {
    ...
    const tasks = [Messaging.addMessageToRoom(roomId, mid, timestamp), Messaging.markRead(uid, roomId), db.sortedSetAdd('messages:mid', timestamp, mid), db.incrObjectField('global', 'messageCount')];
    if (data.toMid) {
    ...
    if (roomData.public) {
      ...
      tasks.push(Messaging.addRoomToUsers(roomId, uids, timestamp), Messaging.markUnread(uids.filter(uid => uid !== String(data.uid)), roomId));
    }
