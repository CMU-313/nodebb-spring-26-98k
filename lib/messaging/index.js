...
Messaging.getMessages = async params => {
  ...
  const [mids, messageCount] = await Promise.all([getMessageIds(roomId, uid, start, stop), db.getObjectField(`chat:room:${roomId}`, 'messageCount')]);
  if (!mids.length) {
    return [];
  }
...
async function getMessageIds(roomId, uid, start, stop) {
  ...
  return await db.getSortedSetRevRangeByScore(`chat:room:${roomId}:mids`, start, stop - start + 1, '+inf', userjoinTimestamp);
}
...
Messaging.getPublicRooms = async (callerUid, uid) => {
  ...
  const checks = await Promise.all(allRoomData.map(room => room && (!Array.isArray(room.groups) || !room.groups.length || isAdmin || groups.isMemberOfAny(uid, room && room.groups))));
  ...
  const unreadCounts = await Promise.all(roomIds.map(async roomId => {
    ...
    const unreadMids = await db.getSortedSetRangeByScore(`chat:room:${roomId}:mids`, 0, maxUnread + 1, cutoff, '+inf');
...
Messaging.getRecentChats = async (callerUid, uid, start, stop) => {
  ...
  async function getUsers(roomIds) {
    const arrayOfUids = await Promise.all(roomIds.map(roomId => Messaging.getUidsInRoom(roomId, 0, 9)));
    const uniqUids = _.uniq(_.flatten(arrayOfUids)).filter(_uid => _uid && parseInt(_uid, 10) !== parseInt(uid, 10));
    const uidToUser = _.zipObject(uniqUids, await user.getUsersFields(uniqUids, ['uid', 'username', 'userslug', 'picture', 'status', 'lastonline']));
...
Messaging.generateUsernames = function (room, excludeUid) {
  ...
  if (users.length > 3) {
    return translator.compile('modules:chat.usernames-and-x-others', usernames.slice(0, 2).join(', '), room.userCount - 2);
  }
...
Messaging.generateChatWithMessage = async function (room, callerUid, userLang) {
  ...
  const usernames = users.map(u => utils.isNumber(u.uid) ? `<a href="${relative_path}/uid/${u.uid}">${u.displayname}</a>` : `<a href="${relative_path}/user/${u.username}">${u.displayname}</a>`);
  let compiled = '';
  if (!users.length) {
    return '[[modules:chat.no-users-in-room]]';
  }
  if (users.length > 3) {
    compiled = translator.compile('modules:chat.chat-with-usernames-and-x-others', usernames.slice(0, 2).join(', '), room.userCount - 2);
  } else {
...
Messaging.getTeasers = async (uid, roomIds) => {
  const mids = await Promise.all(roomIds.map(roomId => Messaging.getLatestUndeletedMessage(uid, roomId)));
  const [teasers, blockedUids] = await Promise.all([Messaging.getMessagesFields(mids, ['fromuid', 'content', 'timestamp']), user.blocks.list(uid)]);
  const uids = _.uniq(teasers.map(t => t && t.fromuid).filter(uid => uid && !blockedUids.includes(uid)));
  const userMap = _.zipObject(uids, await user.getUsersFields(uids, ['uid', 'username', 'userslug', 'picture', 'status', 'lastonline']));
  return await Promise.all(roomIds.map(async (roomId, idx) => {
    ...
    teaser.content = validator.escape(String(utils.stripHTMLTags(utils.decodeHTMLEntities(teaser.content))));
...
Messaging.canMessageUser = async (uid, toUid) => {
  ...
  const [exists, isTargetPrivileged, canChat, canChatWithPrivileged] = await Promise.all([user.exists(toUid), user.isPrivileged(toUid), privileges.global.can('chat', uid), privileges.global.can('chat:privileged', uid), checkReputation(uid)]);
  if (!exists) {
  ...
  const [settings, isAdmin, isModerator, isBlocked] = await Promise.all([user.getSettings(toUid), user.isAdministrator(uid), user.isModeratorOfAnyCategory(uid), user.blocks.is(uid, toUid)]);
  if (isBlocked) {
...
Messaging.canMessageRoom = async (uid, roomId) => {
  ...
  const [roomData, inRoom, canChat] = await Promise.all([Messaging.getRoomData(roomId), Messaging.isUserInRoom(uid, roomId), privileges.global.can(['chat', 'chat:privileged'], uid), checkReputation(uid), user.checkMuted(uid)]);
  if (!roomData) {
...
Messaging.hasPrivateChat = async (uid, withUid) => {
  if (parseInt(uid, 10) === parseInt(withUid, 10) || parseInt(uid, 10) <= 0 || parseInt(withUid, 10) <= 0) {
    return 0;
  }
  const results = await utils.promiseParallel({
...
Messaging.canViewMessage = async (mids, roomId, uid) => {
  ...
  const [midTimestamps, userTimestamp] = await Promise.all([db.sortedSetScores(`chat:room:${roomId}:mids`, mids), db.sortedSetScore(`chat:room:${roomId}:uids`, uid)]);
  const canView = midTimestamps.map(midTimestamp => !!(midTimestamp && userTimestamp && (isPublic || userTimestamp <= midTimestamp)));
